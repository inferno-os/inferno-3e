...
Image_text(void *fp)
{
	...
	locked = lockdisplay(d->display, 0);
	if(str->len >= 0)
		pt = stringn(d, IPOINT(f->p), s, IPOINT(f->sp), font, str->Sascii, str->len);
	else
		pt = runestringn(d, IPOINT(f->p), s, IPOINT(f->sp), font, str->Srune, -str->len);
	checkflush(f->dst);
	if(locked)
		unlockdisplay(d->display);
    Return:
...
void
Display_cursorset(void *fp)
{
	F_Display_cursorset *f;
	Display *d;

	f = fp;
	d = checkdisplay(f->d);
	cursorset(d, *(Point *)&(f->p));
}
...
doflush(Display *d)
{
	...
	if((m = kchanio(d->datachan, d->buf, n, OWRITE)) != n){
		...
		err[0] = 0;
		kgerrstr(err);
		if(_drawdebug || strcmp(err, "screen id in use") != 0 && strcmp(err, "image memory allocation failed") != 0){
