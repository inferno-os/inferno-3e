...
tkenterleave(TkCtxt *c, int x, int y)
{
	...
	m.y = y;
	ot = nil;
	if((otk = c->tkMfocus) != nil) {
		ot = otk->env->top;
		c->tkMfocus = nil;
		tkdeliver(otk, TkLeave, &m);
		tkupdate(ot);
	}
	if(c->tkMgrab != nil && tkdescendant(c->tkMgrab, tk) == 0)
		return;

	if(tk == nil && ot != nil && ot->cursor != nil)
		dcursor(c->screen->display, nil);
	else if(tk != nil && ot != (t = tk->env->top) && (ot == nil || ot->cursor != t->cursor))
		dcursor(c->screen->display, t->cursor);
...
Tk_mouse(void *a)
{
	...
	if(c->tkmstate.x != f->x || c->tkmstate.y != f->y || tkstylus && c->tkmstate.b==0 && f->button&7) {
	...
	lastb = c->tkmstate.b;
	c->tkmstate.b = f->button & 7;		/* Just the buttons */
	...
	if(8 & d) {
		etype = TkButton4R;
		if(m.b & 8)
			etype = TkButton4P|dtype;
		tkdeliver(tk, etype, &m);
	}
	if(16 & d) {
		etype = TkButton5R;
		if(m.b & 16)
			etype = TkButton5P|dtype;
		tkdeliver(tk, etype, &m);
	}
	if(32 & d) {
		etype = TkButton6R;
		if(m.b & 32)
			etype = TkButton6P|dtype;
		tkdeliver(tk, etype, &m);
	}
	if(tkstylus && lastb!=0 && m.b==0){
		TkTop *ot = tk->env->top;
		if(c->tkMfocus != nil){
...
tkreplimg(TkTop *t, Draw_Image *f, Image **ximg)
{
	...
	if(new == nil)
		return;
	draw(new, new->r, cimg, nil, cimg->r.min);
	if(tkwiretap != nil)
...
tkimgcopy(TkTop *t, Image *cimg)
{
	...
	dp = t->display;
	new = allocimage(dp, cimg->r, cimg->ldepth, cimg->repl, 0);
	if(new == nil)
		return H;
...
Tk_imageget(void *a)
{
	...
	if(i == nil) {
		...
		case TKcanvas:
			...
			locked = lockdisplay(d, 0);
			f->ret->t0 = tkimgcopy(t, tkc->image);
			f->ret->t1 = tkimgcopy(t, tkc->mask);
			if(locked)
				unlockdisplay(d);
			return;
		case TKlabel:
