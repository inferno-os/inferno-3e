 ...
 */
File*
filep(FChan *cp, int fid, int flag)
{
	File *f, *prev;
...
newwp(void)
{
	...
	i = si + 1;
	if(i < 0 || i >= fsconf.nwpath)
		i = 0;
	si = i;
	sw = &wpaths[i];
	ew = &wpaths[fsconf.nwpath];
	for(w=sw;;) {
		w++;
		if(w >= ew)
			w = &wpaths[0];
...
formatinit(void)
{
	...
	fmtinstall('Y', Yconv);	/* print channels */
	fmtinstall('Z', Zconv);	/* print devices */
	fmtinstall('T', Tconv);	/* print tags */
}
...
superream(Device dev, long addr)
{
	...
	s = (Superb*)p->iobuf;
	s->sup.fstart = 1;
	s->sup.fsize = devsize(dev);
	s->fbuf.nfree = 1;
	s->sup.qidgen = 10;
	for(i=s->sup.fsize-1; i>=addr+2; i--)
		addfree(dev, i, s);
	putbuf(p);
}
