...
fsctl(char *s)
{
	...
	dev.type = Devwren; /*dev=(Device){Devwren,0,0,0};/* only type for now*/
	dev.ctrl = fs-filesys;
	fs->dev = dev;
	err = nil;
	if (KFS_ISRO) {
...
cprint(char *fmt, ...)
{
	...
	if(1||fscons.out == nil || qclosed(fscons.out) || qfull(fscons.out))
		print("%s",buf); /* putstrn(buf, n);	/* force it out: might be important */
	else
