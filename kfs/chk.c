...
static	long	nfiles;
static	long	maxq;
static	char*	local_calloc;
static	char*	local_callocbase;
static	Device	dev;
static	long	ndup;
static	long	nused;
static	long	nfdup;
...
dalloc(ulong n)
{
	...
	if(local_callocbase == 0)
		local_calloc = local_callocbase = zalloc(MAXDEPTH*sizeof(Dentry));
	p = local_calloc;
	local_calloc += n;
	if(local_calloc > local_callocbase+MAXDEPTH*sizeof(Dentry))
...
check(Filsys *fs, long flag)
{
	...
	if(cwflag) {
		if(amark(sb->sup.roraddr)) {
			/* nothing to do */;
		}
		if(amark(sb->sup.next)) {
			/* nothing to do */;
		}
	}
