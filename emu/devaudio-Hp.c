...
doioctl(int fd, int whim, void *data, char *what)
{
	...
	osleave();
	if (r < 0) {
		n = snprint(ebuf, ERRLEN, "ioctl %s: ", what);
...
static long
audioread(Chan* c, void *ua, long n, ulong offset)
{
	Audiodev *a;
	char buf[300], ebuf[ERRLEN];
	int liv, riv, lov, rov;
	int j, m;
	long path;
...
static long
audiowrite(Chan* c, char *ua, long n, ulong offset)
{
	Audiodev *a;
	long m, n0;
	int i, nf, v, left, right, in, out;
	char buf[255], *field[Ncmd], ebuf[ERRLEN], *p;
