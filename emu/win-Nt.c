...
static void
graphicsgmap(PALETTEENTRY *pal)
{
	int i;

	for(i=0; i < 256; i++){
		pal[i].peRed = pal[i].peGreen = pal[i].peBlue = (255-i)*0x01010101;
		pal[i].peFlags = 0;
	}
}
...
WindowProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	...
	process:
		...
		break;
	case WM_SYSKEYDOWN:
		inLatin = wparam == VK_MENU;
		break;
	case WM_SYSKEYUP:
		if(wparam == VK_MENU && inLatin){
...
dotranslate(MSG *msg)
{
	/* prevent WM_KEYDOWN:VK_TAB from generating WM_CHAR:'\t' */
