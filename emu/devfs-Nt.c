...
static	void		fsdirset(char*, WIN32_FIND_DATA*, char*, int, Rune*);
...
fsattach(char *spec)
{
	...
	if (!NIL(drive) && (!globfs || drive[1] != ':' || drive[2] != '\0'))
		error(Ebadspec);
	...
	FS(c)->srv = ntsrv;
	if (!NIL(spec)) {
		char *s = malloc(strlen(spec)+1);
		strcpy(s, spec);
		FS(c)->spec = s;
		FS(c)->srv = filesrv(spec);
		c->qid.path = fsqidpath(spec)|CHDIR;
		c->qid.vers = 0;
	}
	FS(c)->name = newcname("/");
	return c;
...
fscreate(Chan *c, char *name, int mode, ulong perm)
{
	...
	if(usesec) {
		...
		*p = '\\';
		...
		if(checksec && !secsdhasperm(sd, WMODE, FS(c)->srv)
...
fsstat(Chan *c, char *buf)
{
	...
	if(fsisroot(c)){
		...
		strcpy(data.cFileName, fslastelem(FS(c)->name));
	}

	fsdirset(buf, &data, path, c->dev, FS(c)->srv);
}
...
fspath(Cname *c, char *ext, char *path, char *spec)
{
	...
	if(strlen(rootd) + extlen >= MAX_PATH) error(Etoolong);
	strcpy(path, rootd);
	if(ext) extlen = strlen(ext) + 1;
	if(cnisroot(c)){
		if(ext) {
			strcat(path, "\\");
			strcat(path, ext);
		...
		if(*(c->s) != '/') {
			if(strlen(path) + 1 >= MAX_PATH) error(Etoolong);
			strcat(path, "\\");
		}
		if(strlen(path) + strlen(c->s) + extlen >= MAX_PATH) error(Etoolong);
		strcat(path, c->s);
		if(ext){
			strcat(path, "\\");
			strcat(path, ext);
...
fsdirread(Chan *c, uchar *va, int n, ulong offset)
{
	...
	do {
		...
		path[MAX_PATH-1] = '\0';
		fsdirset(va+i, &data, path, c->dev, FS(c)->srv);
		i += DIRLEN;
		FS(c)->offset += DIRLEN;
...
static void
fsdirset(char *edir, WIN32_FIND_DATA *data, char *path, int dev, Rune* srv)
{
	Dir dir;
	char ebuf[ERRLEN];
