...
proggen(Chan *c, Dirtab *tab, int ntab, int s, Dir *dp)
{
	...
	if(s == DEVDOTDOT){
		qid.path = CHDIR;
		qid.vers = 0;
		devdir(c, qid, "#p", 0, eve, CHDIR|0555, dp);
		return 1;
	}
...
progwalk(Chan *c, char *name)
{
	 ...
	 */
	if(c->qid.path == CHDIR && strcmp(name, "..") != 0) {
...
progfds(Osenv *o, char *va, int count, long offset)
{
	...
	for(i = 0; i <= f->maxfd; i++) {
		...
		if(c == nil)
			continue;
		n += snprint(va+n, count-n, "%3d %.2s %C %4ld %.8lux.%.8ld %8ld ",
			i,
			&"r w rw"[(c->mode&3)<<1],
...
static int
progenv(Egrp *e, char *va, int count, long offset)
{
	int n = 0;
	Evalue *el;
	char buf[256];

	for (el = e->entries; el != nil; el = el->next) {
		strncpy(buf, el->val, el->len);
		buf[el->len] = '\0';
		n += snprint(va+n, count-n, "%s=%s\n", el->var, buf);
		if(offset > 0) {
			offset -= n;
			if(offset < 0) {
				memmove(va, va+n+offset, -offset);
...
progread(Chan *c, void *va, long n, ulong offset)
{
	...
	case Qns:
		...
		n -= i;
		a += i;
		i = readstr(0, a, n, mw->mh->from->name->s);
		n -= i;
		a += i;
		if(n > 0) {
			*a++ = ' ';
		...
		i = readstr(0, a, n, mw->cm->to->name->s);
		poperror();
		release();
		n -= i;
		a += i;
...
dbgxec(Prog *p)
{
	...
	while(R.IC != 0) {
		...
		if(op == ILOAD) {
			n = snprint(buf, ERRLEN, "load %s", string2c(*(String**)R.s));
			qproduce(ctl->q, buf, n);
			buf[0] = '\0';
		}
