...
attachscreen(Rectangle *r, int *ld, int *width, int *softscreen)
{
	...
	mousepid = kproc("readmouse", brazilreadmouse, nil, 0);
	keybdpid = kproc("readkbd", brazilreadkeybd, nil, 0);
...
ldepthof(char *s)
{
	...
	case 32:
		...
		for(i=0; i<256; i++){
			...
			grn = (rgb>>8)&0xFF;
			blu = (rgb>>0)&0xFF;
			remap[i] = (0xFF<<24)|(red<<16)|(grn<<8)|blu;
		}
		return 3;
	case 24:
		/* RGB24 = 8 8 8 */
		for(i=0; i<256; i++){
			...
			grn = (rgb>>8)&0xFF;
			blu = (rgb>>0)&0xFF;
			remap[i] = (red<<16)|(grn<<8)|blu;
		}
		return 3;
	case 16:
		if(size[ldR]!=0&&size[ldG]!=0&&size[ldB]!=0){
			for(i=0; i<3; i++){
				size[i] = 8 - size[i];
				shift[i] = 16 - shift[i];
			}
			for(i=0; i<256; i++){
				rgb = cmap2rgb(i);
				red = ((rgb>>16)&0xFF) >> size[ldR];
				grn = ((rgb>>8)&0xFF) >> size[ldG];
				blu = ((rgb>>0)&0xFF) >> size[ldB];
				remap[i] = (red<<shift[ldR])|(grn<<shift[ldG])|(blu<<shift[ldB]);
			}
			return 3;
		}
		/* RGB16 = 5 6 5 */
...
brazilloadimage(Rectangle r, uchar *data, int ndata)
{
	...
	ndata = 0;
	scale = 1;
	if(newkernel && truedepth>8)
		scale = truedepth/8;
	while(r.max.y > r.min.y){
		dy = r.max.y - r.min.y;
		if(dy*bpl*scale > CHUNK)
			dy = CHUNK/(bpl*scale);
		n = dy*bpl;
		chunk[0] = 'w';
		if(newkernel)
...
setcursor(Point pt)
{
	...
	pt.x += tiler.min.x;
	pt.y += tiler.min.y;
	sprint(buf, "m%11d %11d %11d %11d ", pt.x, pt.y, 0, 0);
...
mapspecials(char *s1, char *s2, int *n)
{
	...
	while (fullrune(s, es2 - s)) {
		...
		case Khome:
			...
			break;
		/*
		Kend is '\r' which we don't really want to remap
		case Kend:
			r = End;
			break;
		*/
