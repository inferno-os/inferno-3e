...
ipgen(Chan *c, Dirtab *d, int nd, int s, Dir *dp)
{
	...
	if(s == DEVDOTDOT){
		...
		case Qtopdir:
		case Qprotodir:
			q.path = QID(0, 0, Qtopdir)|CHDIR;
			devdir(c, q, "#I", 0, "network", 0555, dp);
			break;
		case Qconvdir:
			s = PROTO(c->qid);
...
eipfmt(Fmt *f)
{
	...
	case 'I':		/* v4 ip address */
	case 'V':
		p = va_arg(f->args, uchar*);
		return fmtprint(f, ifmt, p[0], p[1], p[2], p[3]);
	case 'i':
		hnputl(ip, va_arg(f->args, ulong));
		return fmtprint(f, ifmt, ip[0], ip[1], ip[2], ip[3]);
	}
