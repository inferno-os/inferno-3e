...
#define	QID(q)		(((ulong)(q).path&0x00000007)>>0)
...
static	void		drawmesg(Client*, void*, int);
static	void		drawuninstall(Client*, int);
...
drawreset(void)
{
	screenimage.data = &screendata;	/* Used by X only */
}

void
drawend(void)
{
	reattach = 2;	/* Used by Nt only */
}
...
drawattach(char *spec)
{
	...
	if(screendata.data == nil || reattach-- == 1){
		screendata.base = nil;
...
drawmesg(Client *client, void *av, int n)
{
	...
	while((n-=m) > 0){
		...
		case 'P':
			...
			ox = oy = 0;
			u = a+m;
			br = Rect(10000, 10000, -10000, -10000);
			for(y=0; y<ni; y++){
