...
struct
{
	...
	3,
	{
		{ "main",  0, 	32*1024*1024, 31,  512*1024, 0 },
		{ "heap",  1, 	32*1024*1024, 31,  512*1024, 0 },
...
poolalloc(Pool *p, ulong asize)
{
	...
	while(t) {
		if(t->size == size) {
			...
			unlock(&p->l);
			if(p->monitor)
				MM(p->pnum, getcallerpc(&p), (ulong)B2D(t), size);
			return B2D(t);
		}
		if(size < t->size) {
			q = t;
	...
	if(q != nil) {
		...
		if(frag < (size>>2)) {
			...
			unlock(&p->l);
			if(p->monitor)
				MM(p->pnum, getcallerpc(&p), (ulong)B2D(q), size);
			return B2D(q);
		}
		/* Split */
		ns = q->size - size;
...
free(void *v)
{
	...
	if(v != nil) {
		if(Npadlong)
			v = (ulong*)v-Npadlong;
		D2B(b, v);
		MM(1<<8|0, getcallerpc(&v), (ulong)v, b->size);
		poolfree(mainmem, v);
	}
}
