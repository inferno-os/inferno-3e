...
release(void)
{
	...
	*pq = p->qnext;
	...
	erendezvous(p, 0);		/* wake up thread to run VM */
...
iyield(void)
{
	...
	unlock(&isched.l);
	erendezvous(p, 0);		/* wake up acquiring kproc */
	strcpy(up->text, "yield");
	erendezvous(up, 0);		/* sleep */
	strcpy(up->text, "dis");
}
...
progexit(void)
{
	...
	if(up->env->debug != nil) {
		...
		broken = 1;
		/* must keep it if in debug */
	}else if(broken && (strcmp(estr, exHeap) == 0 || strcmp(estr, exNomem) == 0 || strcmp(estr, Enomem) == 0))
		broken = 0;	/* don't keep it if out of memory */
...
disfault(void *reg, char *msg)
{
	...
	/* cause an exception on the dis prog.  As for error(), but Plan 9 needs reg*/
