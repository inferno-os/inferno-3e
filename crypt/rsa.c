...
void
freeRSAPrivateKey(void *vpk)
{
	RSAPrivateKey *pk = vpk;
...
RSAEncrypt(BigInt message, RSAPublicKey *key)
{
	...
	if(bigCompare(message, key->modulus) >= 0)
		print("RSAEncrypt: message too long\n");
...
RSADecrypt(BigInt message, RSAPrivateKey *key)
{
	...
	if(bigCompare(message, key->modulus) >= 0)
		print("RSADecrypt: message too long\n");

	p = (RSAPublicKey *)malloc(sizeof(RSAPublicKey));
	p->publicExponent = key->publicExponent;
	p->modulus = key->modulus;
...
void *
RSASign(BigInt message,  void *vkey)
{
	RSAPrivateKey *key = vkey;

	return (void *)RSADecrypt(message, key);
}


Boolean
RSAVerify(BigInt message, void *vsig, void *vkey)
{
	RSASignature *sig = vsig;
	RSAPublicKey *key = vkey;
	Boolean retval;
	BigInt cmp;
