...
static void stackcheck(int, Prog*);
...
#undef	R
		...
		case AMODU:
			...
			if(dynld){
				if(!dyncall(div, 1) && div->to.sym->type == SXREF)
					diag("undefined: %s\n", div->to.sym->name);
				if(seenthumb)
					div->link->link->link->as = ABX;
				p = q1;
			}
		...
		if(dynld && p->as != ATEXT){
			...
			if(isfnptr(&p->from) && dynfn(p->from.sym) >= 0)
				diag("cannot use interface function %s as a function pointer", p->from.sym->name);
			else if((df = isdata(&p->from)) || isdata(&p->to)){
				if(thumb){
					r = REGTMPT;
					q = prg();
					*q = *p;
				...
				if(df){
					q->from.offset += q->from.sym->value;
					q->from.reg = r;
					p->from.sym = q->from.sym = S;
					p->from.name = q->from.name = D_NONE;
				}
				else{
					q->to.offset += q->to.sym->value;
					q->to.reg = r;
					p->to.sym = q->to.sym = S;
					p->to.name = q->to.name = D_NONE;
				}
				if(!df && thumb && (q->to.offset < 0 || q->to.offset > 255)){
					p->from.type = D_CONST;
					p->from.offset = q->to.offset;	// movw $O, R7
					...
					q1->from.reg = REGDS;
					q1->to.type = D_REG;
					q1->to.reg = REGTMPT;		// add R9, R7
					p->link = q1;
					q1->link = q;

					q->to.offset = 0;			// mov* Rs, (R7)
				}
				p = q;
			}
		}
...
*/
	if(stkfn == S){
		...
		if(s->type != STEXT && s->type != SLEAF)
			diag("__stkflt not defined");
		stkfn = s;
		if(seenthumb)
			s->foreign = 1;
		for(q = firstp; q != P; q = q->link){
			if(q->as == ATEXT && q->from.sym == s){
				stkprg = q;
				break;
			}
		}
	...
	if(curtext != P && curtext->from.sym == stkfn)
		return;
