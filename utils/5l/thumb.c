		...
		case D_STATIC:
			...
			if(s->type == STEXT || s->type == SLEAF){
				...
				if(s->thumb)
					instoffset++;	// T bit
#endif
				if(dynld && p->as != AWORD && (a != &p->from || p->to.type != D_REG))
					diag("help %P: fn ptr %s not movw $fp, r", p, s->name);
				return C_LCON;
			}
			if(dynld)
				diag("bad dynld D_OCONST");
...
mv(Prog *p, int r, int off)
{
	...
	if(p != nil && p->cond != nil){	// in literal pool
		...
		numr(p, off, 0, 255);
		o = 0x9<<11;
		if(dynld && isfnptr(&p->from))
			dynreloc(p->cond->pc, 1, p->from.sym);
	}
	else{
		numr(p, off, 0, 255);
		o = 0x4<<11;
