...
#define	setarch(p)		if((p)->as==ATEXT) thumb=(p)->reg==0
#define	setthumb(p)	if((p)->as==ATEXT) seenthumb|=(p)->reg==0
...
void	doprof1(void);
void	doprof2(void);
int	dyncall(Prog*, int);
long	dynentry(Sym*);
int	dynfn(Sym*);
void	dynreloc(long, int, Sym*);
