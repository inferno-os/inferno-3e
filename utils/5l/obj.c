...
main(int argc, char *argv[])
{
	...
	case 'm':
		module = ARGF();
		dynld = 1;
		outfile = malloc(strlen(module)+3);
		strcpy(outfile, module);
		strcat(outfile, ".o");
		lowercase(outfile);
		INITTEXT = INITDAT = INITRND = 0;
		HEADTYPE = 2;
		break;
	case 'i':
		kiffile = ARGF();
...
zaddr(uchar *p, Adr *a, Sym *h[])
{
	...
	if(a->reg == notreg)
		print("%s: register %d in use !\n", curtext != P ? curtext->from.sym->name : "", notreg);
...
loop:
	...
	if(p->reg == notreg)
		print("%s: register %d in use !\n", curtext != P ? curtext->from.sym->name : "", notreg);
	if(p->as == AMOVM && notreg >= 0)
		if((p->from.type == D_CONST && (p->from.offset & (1<<notreg))) ||
		    (p->to.type == D_CONST && (p->to.offset & (1<<notreg))))
			print("%s: register %d used in movm !\n", curtext != P ? curtext->from.sym->name : "", notreg);
