...
remget(struct segment *s, ulong addr, long off, char *buf, int size)
{
	...
	if (protodebug)
		fprint(2, "remget addr x%lx off %lx\n", addr, off);
	for (t = 0; t < size; t += n) {
		n = size;
		if(n > 9)
			n = 9;
...
remput(struct segment *s, ulong addr, long off, char *buf, int size)
{
	...
	if (protodebug)
		fprint(2, "remput addr x%lx off %lx\n", addr, off);
	for (t = 0; t < size; t += n) {
		n = size;
		if(n > 4)
			n = 4;
...
remcondset(char op, ulong val)
{
	...
	if (protodebug)
		fprint(2, "remcondset op %c val: %lx\n", op, val);
	memset(dbg, 0, sizeof(dbg));
...
rproto(int fd, char *buf, int nb)
{
	...
	if (protodebug) {
		int i;
		print("rproto remote write fd %x bytes: %d\n", fd, nb);
		for (i=0; i < nb; i++) {
			print(" %2.2x", buf[i]);
		}
		print("\n");
...
myreadn(int f, void *av, long n)
{
	...
	if (protodebug) {
		print("remote read fd %x bytes: %ld", f, n);
	}
	a = av;
	t = 0;
	while(t < n){
