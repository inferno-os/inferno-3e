...
static int ntcompile = 0;
static char* ntcomstr = "NT_COMPILE=1";
...
outfun(Node *n)
{
	if(n == Z || n->op != OFUNC)
		return;
	n = n->left;
	if(n->op != ONAME || n->sym == nil){
		prtree(n, "not name");
		return;
	}
	if(!outinit){
		if(ntcompile)
			print("#define NT_COMPILE	1\n\n");
		print("#include \"%s\"\n\n", outf);
		print("/* automatically generated by kif */\n\n");
		print("typedef struct Kiftab Kiftab;\n\n");
		print("struct Kiftab{\n");
		print("\tvoid (*fn)(void*);\n");
		print("\tchar *str;\n");
		print("\tKiftab *nxt;\n");
