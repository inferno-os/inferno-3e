...
typecheck(int checkimp)
{
	...
	if(checkimp){
		...
		if(impdecl->store != Dtype || impdecl->ty->kind != Tmodule){
			error(impdecl->src.start, "cannot implement %K", impdecl);
			return nil;
		}
 ...
 */
Ok
echeck(Node *n, int typeok, int isglobal)
{
	Type *t, *tt;
	Node *left, *right, *mod;
	Decl *tg, *id, *callee;
	...
	switch(n->op){
	case Odas:
		kidsok = echeck(right, 0, isglobal);
		if(!kidsok.ok)
			right->ty = terror;
...
casecheck(Node *cn, Type *ret)
{
	...
	t = arg->ty;
	if(t != tint && t != tstring){
		nerror(cn, "case argument %Q is not an int or string", arg);
		return;
	}
