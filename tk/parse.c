...
pdist(TkTop *t, TkOption *o, void *place, char **str, char *buf, char *ebuf)
{
	...
	if (o->aux == nil)
		...
		e = OPTION(place, TkEnv*, AUXI(o->aux));
	p = tkfrac(t, *str, &d, e);
	if(p == nil)
		return TkBadvl;
	*str = p;
...
pevim(TkTop *t, TkOption *o, void *place, char **str, char *buf, char *ebuf)
{
	...
	*str = tkword(t, *str, buf, ebuf, nil);
	...
	if(i != nil) {
		...
		draw(cimg, cimg->r, i->fgimg, nil, ZP);
		e->evim[col] = cimg;
	}
out:
	if(locked)
...
static void
changecol(TkEnv *e, int setcol, int col, int rgb, int delta)
{
	int val, R, G, B;
...
pcolr(TkTop *t, TkOption *o, void *place, char **str, char *buf, char *ebuf)
{
	...
	*str = tkskip(*str, " \t");
	...
	color = AUXI(o->aux);
	changecol(e, setcol, color, rgb, 0);
	if(color == TkCbackgnd || color == TkCselectbgnd || color == TkCactivebgnd) {
		changecol(e, setcol, color+1, rgb, Tkshdelta);
		changecol(e, setcol, color+2, rgb, -Tkshdelta);
	}
...
pfrac(TkTop *t, TkOption *o, void *place, char **str, char *buf, char *ebuf)
{
	...
	*str = tkword(t, *str, buf, ebuf, nil);
	...
	if(item == nil)
		return TkNomem;
	for(i = 0; i < n; i++) {
		p = tkword(t, p, item, item+Tkmaxitem, nil);
		if(*item == '\0') {
			free(item);
			return TkOparg;
		}
