...
tkcvsovaldraw(Image *img, TkCitem *i, TkEnv *pe)
{
	...
	if(pen != nil) {
		...
		else if (tkmkmask(o->canv, img, i, &org)) {
			mc = subpt(c, org);
			fillellipse(o->canv->mask, mc, dx, dy, o->stipple, c);
			gendraw(img, i->p.bb, pen, i->p.bb.min, o->canv->mask, subpt(i->p.bb.min, org));
		}
	}
