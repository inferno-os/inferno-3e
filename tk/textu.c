...
tktextevent(Tk *tk, int event, void *data)
{
	...
	if(event == TkLeave && tkt->mouse != nil) {
		vm.x = 0;
		vm.y = 0;
		tktdeliver(tk, tkt->mouse, tkt->mouse, TkLeave, data);
		tkt->mouse = nil;
		tkt->focus = nil;
	}
		...
		if(tkt->mouse != f) {
			...
			if(tkt->mouse != nil) {
				...
				tktdeliver(tk, tkt->mouse, tagit, TkLeave, data);
				if(tagit)
					free(tagit);
				tagit = nil;
			}
			...
			tktdeliver(tk, f, tagit, TkEnter, data);
			tkt->mouse = f;
			if(tagit)
				free(tagit);
		}
		if(m.b & 1) {
			if(f->kind == TkTwin && f->iwin->sub != nil) {
				tkt->focus = f;
				tktdeliver(tk, f, f, TkFocusin, data);
			}
			else {
				if(tkt->focus != nil && tkt->focus->kind == TkTwin &&
						tkt->focus->iwin->sub != nil)
					tktdeliver(tk, tkt->focus, tkt->focus, TkFocusout, data);
				tkt->focus = nil;
			}
		}
		if(tkt->mouse != nil)
