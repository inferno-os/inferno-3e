...
tkentrytext(Tk *tk, Image *i, Rectangle s, TkEntry *e, TkEnv *env)
{
	...
	if(e->sell > e->self) {
		...
		tktextsdraw(i, r, env, tk->sborderwidth);
	}
	...
	if(posn != nil && len > 0) {
		...
		if (e->sell > e->self) {
			...
			if (start < end)
				runestringn(i, r.min, tkgc(env, TkCselectfgnd), r.min, env->font, posn+start, end-start);
		}
	}
	...
	if(c->tkKgrab == tk) {
		...
		if(e->icursor >= ws && e->icursor <= ws+len) {
			...
			draw(i, r, tkgc(env, TkCforegnd), nil, ZP);
		}
	}
...
tkdrawentry(Tk *tk, Point orig)
{
	...
	draw(i, r, tkgc(env, TkCbackgnd), nil, ZP);
	...
	tkdrawrelief(i, tk, ZP, Frameinset, TkCbackgnd);
...
tkentryinsert(Tk *tk, char *arg, char **val)
{
	...
	tkword(top, arg, text, text+Tkentryins, nil);
	n = utflen(text);
...
tkentryget(Tk *tk, char *arg, char **val)
{
	...
	rbuf = (Rune *)mallocz((last - first + 1) * sizeof(Rune), 0);
	if(rbuf == nil)
		return TkNomem;
	memmove(rbuf, ent->text+first, (last-first)*sizeof(Rune));
	rbuf[last - first] = L'\0';
	r = tkvalue(val, "%S", rbuf);
	free(rbuf);
	return r;
}
