...
#define D if (!strcmp(tk->name->name, ".t")) 	/* XXX remove */
...
static TkEbind tktbinds[] = {
	{TkButton1P,		"grab set %W; %W tkTextButton1 %x %y"},
...
//print("tktsetdeltas, orig: %d %d -> iv: %d %d; sv: %d %d\n",
//	orig.x, orig.y,
//	tkt->deltaiv.x, tkt->deltaiv.y,
//	tkt->deltasv.x, tkt->deltasv.y);
}
		...
		if(tktanytags(it)) {
			...
			if((o == TKsunken || o == TKraised) && bd > 0) {
				...
				if(join) {
					...
					r.min.y = r.max.y;
					r.max.y = r.min.y + bd;
					draw(i, r, tkgc(et, bevbot), nil, ZP);
				}
				...
				if(z != nil && tktsametags(z, it)) {
					...
					r.min.y = q.y;
					r.max.y = q.y + bd;
					draw(i, r, tkgc(et, bevtop), nil, ZP);
					r.min.y = r.max.y;
					r.max.y = r.min.y + h;
					draw(i, r, bg, nil, ZP);
					join = 1;
...
mprint(Mprint *m, char *fmt, ...)
{
	...
	va_start(arg, fmt);
	new = vseprint(m->buf+m->ptr, m->buf+m->len, fmt, arg);
	va_end(arg);
	m->ptr = new - m->buf;
	return m->ptr;
}
...
tktextconfigure(Tk *tk, char *arg, char **val)
{
	...
	if (tkt->propagate == BoolT) {
		/*
		 * if propagation's on, then we need to fix
		 * geom to make sure that we request the correct width
		 * and height
		 */
