...
tkscalehoriz(Tk *tk, Image *i)
{
	...
	o.x += ScaleBW;
	if(tks->tick > 0){
		int j, t = tks->tick, l = tks->to-tks->from;
		
		r.min.y = o.y+sh+2*ScaleBW;
		r.max.y = r.min.y+3;
		for(j = 0; j <= l; j += t){
			r.min.x = o.x+(j*w)/l+tks->sl/2;
			r.max.x = r.min.x+1;
	...
	if(tk->flag & Tkactivated) {
		...
		r.max.x = o.x+tks->sl;
		r.max.y = o.y+sh;
		draw(i, r, tkgc(e, TkCactivebgnd), nil, ZP);
	}
	switch(tks->relief) {
	case TKsunken:
...
tkscalemotion(Tk *tk, char *arg, char **val)
{
	...
	if(v != slider || z[0] < tks->center-tks->sw || z[0] > tks->center+tks->sw)
		tk->flag &= ~Tkactivated;
	else if(tkstylus == 0 || tk->env->top->ctxt->tkmstate.b != 0)
		tk->flag |= Tkactivated;
