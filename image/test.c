...
Image 	*blue, *red, *yellow, *green, *black, *plaid, *box, *ones;
...
*/
	...
	draw(plaid, Rect(0, 1, 13, 2), yellow, ones, ZP);
	/* vertical black line */
	draw(plaid, Rect(2, 0, 4, 5), black, ones, ZP);

	cross = allocimage(display, Rect(0, 0, 7, 6), 3, 1, 0);
	/* horizontal line */
	draw(cross, Rect(0, 0, 7, 2), ones, ones, ZP);
	/* vertical line */
	draw(cross, Rect(0, 0, 2, 6), ones, ones, ZP);
	...
	lr = allocwindow(screen, R(75, 75, 320-75, 240-75), drawred, 0);
	if(lr == 0){
	...
	lockdisplay(display);
	line(lg, lg->r.min, lg->r.max, 7, 0, blue);
	flushimage(display, 1);
	unlockdisplay(display);
	sleep(1000);
