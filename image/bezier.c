...
bezier(Image *dst, Point p0, Point p1, Point p2, Point p3, int end0, int end1, int radius, Image *src, Point sp)
{
	...
	if(l.np == -1)
		return 0;
	if(l.np != 0){
		poly(dst, l.p, l.np, end0, end1, radius, src, addpt(subpt(sp, p0), l.p[0]));
		free(l.p);
	}
	return 1;
}
...
bezspline(Image *dst, Point *pt, int npt, int end0, int end1, int radius, Image *src, Point sp)
{
	...
	if(l.np==-1)
		return 0;
	if(l.np != 0){
		poly(dst, l.p, l.np, end0, end1, radius, src, addpt(subpt(sp, pt[0]), l.p[0]));
		free(l.p);
	}
	return 1;
}
...
fillbezier(Image *dst, Point p0, Point p1, Point p2, Point p3, int w, Image *src, Point sp)
{
	...
	if(l.np == -1)
		return 0;
	if(l.np != 0){
		fillpoly(dst, l.p, l.np, w, src, addpt(subpt(sp, p0), l.p[0]));
		free(l.p);
	}
	return 1;
}
...
fillbezspline(Image *dst, Point *pt, int npt, int w, Image *src, Point sp)
{
	...
	if(l.np == -1)
		return 0;
	if(l.np > 0){
		fillpoly(dst, l.p, l.np, w, src, addpt(subpt(sp, pt[0]), l.p[0]));
		free(l.p);
	}
	return 1;
}
