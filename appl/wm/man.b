...
tkconfig := array [] of {
	"frame .input",
	"frame .view",
	"text .view.t -state disabled -width 500 -height 400 -bg white -wrap none -yscrollcommand {.view.scroll set}",
	"scrollbar .view.scroll -command {.view.t yview}",
	...
	"pack .title.t -expand 1 -fill x",
	"pack .view.scroll -fill y -side left",
...
init(ctxt : ref Draw->Context, argv : list of string)
{
	...
	wmlib->init();
	buts := Wmlib->Resize | Wmlib->Hide;
	winctl : chan of string;
	(window, winctl) = wmlib->titlebar(ctxt.screen, nil, "Man", buts);
	nav := chan of string;
	plumb := chan of string;
	tk->namechan(window, nav, "nav");
	tk->namechan(window, plumb, "plumb");
	wmlib->tkcmds(window, tkconfig);
	fittoscreen(window);
	tkcmd(window, "update");
	mktags();
	...
	actionchan := chan of ref History;
...
search(pat : string)
{
	...
	if (start == "")
		start = "1.0";
	r := tkcmd(window, ".view.t search " + dir + " -- " + wmlib->tkquote(pat) + " " + start);
	if (r != nil) {
...
setbuttons()
{
	if (history.prev == nil)
		...
		tkcmd(window, ".input.back configure -state active");
	if (history.next == nil)
		tkcmd(window, ".input.forward configure -state disabled");
	else
		tkcmd(window, ".input.forward configure -state active");
}
...
dolayout(linechan : chan of list of (int, Parseman->Text), path : string)
{
	...
	viewman := load Viewman SELF;
	parser->parseman(fd, metrics, 0, viewman, linechan);
}
...
setline(line : list of (int, Parseman->Text))
{
	...
	for (l = line; l != nil; l = tl l) {
		...
		if (text.font != curfont || text.link != curlink || text.heading != curheading || gtag != curgtag) {
			# need to change tags
			linestr = linestr + " " + wmlib->tkquote(fonttext) + " {" + ttag + curgtag + fonttag(curfont, curheading) + "}";
			ttag = "";
			curgtag = gtag;
			fonttext = "";
			curfont = text.font;
...
tabtag(tabstr : string) : string
{
	...
	lasttabstr = tabstr;
	tkcmd(window, ".view.t tag configure " + lasttagname + " -tabs " + wmlib->tkquote(tabstr));
	return lasttagname;
}
...
{
	cmd := wmlib->tkquote("send nav l" + search);
