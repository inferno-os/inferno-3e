...
{
	(t, titlectl) := wmlib->titlebar(ctxt.screen, SETFONT, Name, Wmlib->Appl);
	...
	c := <-cmd =>
		...
		* =>
		...
		"File" =>
			filemenu(t, 0, 0);
		"new" =>
			(name, ok, nil) := getfilename(ctxt.screen, t, "file for new window", f.name, 1, 0, 0);
			if(ok)
...
brutus(ctxt: ref Context, filename: string)
{
	...
	menu := <-titlectl =>
		...
		"exit" =>
			if(f.dirty){
				action := confirm(ctxt.screen, t, nameof(f)+" is dirty", 1);
				case action {
				"cancel" =>
					continue;
				"exitclean" =>
	...
	command := <-cmd =>
		...
		* =>
		...
		"open" =>
			...
			(name, ok, nil) := getfilename(ctxt.screen, t, "file for this window", f.name, 1, 0, 0);
			if(ok && name!=""){
				setfilename(f, name);
		...
		"name" =>
			(name, ok, nil) := getfilename(ctxt.screen, t, "remembered file name", f.name, 1, 0, 0);
			if(ok){
				if(name != f.name){
					setfilename(f, name);
					dirty(f, 1);
		...
		"write" =>
			(name, ok, sgml) := getfilename(ctxt.screen, t, "file to write", f.name, 1, 1, f.fontsused);
			if(ok && name!=""){
				if(f.name == ""){
					setfilename(f, name);
					dirty(f, 1);
		...
		"fonts" =>
			if(f.fontsok==0 && f.fontsused==0){
				action := confirm(ctxt.screen, t, "Converting "+nameof(f)+" to SGML", 0);
				case action {
				"cancel" =>
					continue;
				"exitdirty" =>
		...
		"addext" =>
			ext := hd tl c;
			(args, ok, nil) := getfilename(ctxt.screen, t, "parameters for "+ext, "", 0, 0, 0);
			if(ok){
...
#	good question
	...
	if(f.fontsused == 0)
		...
		tkcmd(t, ".ft.t insert insert "+wmlib->tkquote(snarftext)+" "+f.fonttag);
	else
		insert(f, snarfsgml);
...
insert(f: ref File, sgml: string)
{
	...
	for(i:=0; i<end; i=j){
		...
		if(j > i)
			tkcmd(t, ".ft.t insert insert "+wmlib->tkquote(sgml[i:j])+" "+tt);
		if(j < end)
			case sgml[j] {
			'&' =>
...
mkextension(f: ref File, mod: Brutusext, modname, args: string, taglist: list of string)
{
	...
	mod->init(sys, draw, bufio, tk, wmlib);
...
dumpfile1(f: ref File, file: string, sgml: int): int
{
	if(writefile(f, file, sgml) < 0){
		wmlib->dialog(f.tk, "error -fg red",
			"Write file",
...
confirm(screen: ref Draw->Screen, parent: ref Tk->Toplevel, message: string, write: int): string
{
	s := confirm1(screen, parent, message, write);
	tkcmd(parent, FOCUS);
	return s;
}

confirm1(screen: ref Draw->Screen, parent: ref Tk->Toplevel, message: string, write: int): string
{
...
getfilename(screen: ref Draw->Screen, parent: ref Tk->Toplevel, message, name: string, browse, sgml, nowsgml: int): (string, int, int)
{
	(s, i, issgml) := getfilename1(screen, parent, message, name, browse, sgml, nowsgml);
	tkcmd(parent, FOCUS);
	return (s, i, issgml);
}

getfilename1(screen: ref Draw->Screen, parent: ref Tk->Toplevel, message, name: string, browse, sgml, nowsgml: int): (string, int, int)
{
	...
	for(;;){
		...
		"browse" =>
			...
			path = wmlib->filename(screen, parent, message, pat, dir);
			if(path != "")
				name = path;
