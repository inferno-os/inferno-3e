...
init(ctxt: ref Draw->Context, nil: list of string)
{
	...
	wmlib->init();
	(win, wmcmd) := wmlib->titlebar(ctxt.screen, "", "Mine Sweeper", Wmlib->Hide);
	mainwin = win;
...
reset_display()
{
	for (i := 1; i <= SZB; i++) {
		for (j := 1; j <= SZB; j++) {
			s := sys->sprint(".f%d.b%dx%d configure -text { } -background #dddddd", i, i, j);
			cmd(mainwin, s);
		}
	}
	cmd(mainwin, "update");
...
display_square(i, j: int, v: string, c: string) {
	cmd(mainwin, sys->sprint(".f%d.b%dx%d configure -text {%s} -background %s", i, i, j, v, c));
	cmd(mainwin, "update");
}
