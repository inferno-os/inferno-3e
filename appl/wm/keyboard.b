...
init(ctxt: ref Draw->Context, args: list of string)
{
	...
	(t, wcmd) := wmlib->titlebar(ctxt.screen, "", "Kbd", winopts);
	tk->cmd(t, "frame . -bd 0 -relief flat");

	for(i := 0; i < len keys[0]; i++)
		if(keys[0][i] != nil)
			tk->cmd(t, sys->sprint("button .b%d -font %s -width %d -height %d -bd %d -activebackground %s -text {%s} -command 'send keypress %d",
				i, FONT, KEYSIZE, KEYSIZE, KEYBORDER, background, keys[0][i], keyvals[0][i]));

	for(i = 0; i < len specials; i++) {
		k := specials[i];
		for(xl := k.x; xl != nil; xl = tl xl)
			tk->cmd(t, sys->sprint(".b%d configure -font %s -width %d", hd xl, SPECFONT, k.size));
	}
	...
	for(j:=0; i < len keys[0]; j++){
		...
		tk->cmd(t, "frame "+rowf);
		tk->cmd(t, sys->sprint("frame .pad%d -height %d", j, KEYGAP));
		if(ENDGAP){
			tk->cmd(t, rowf + ".pad -width " + string ENDGAP);
			tk->cmd(t, "pack " + rowf + ".pad -side left");
		}
		for(; keys[0][i] != nil; i++){
			label := keys[0][i];
			expand := label != "\\\\" && len label > 1;
			tk->cmd(t, "pack .b" + string i + " -in "+ rowf + " -side left -fill x -expand "+string expand);
			if(keys[0][i+1] != nil && KEYGAP > 0){
				padf := sys->sprint("%s.pad%d", rowf, i);
				tk->cmd(t, "frame " + padf + " -width " + string KEYGAP);
				tk->cmd(t, "pack " + padf + " -side left");
			}
		}
		if(ENDGAP){
			padf := sys->sprint("%s.pad%d", rowf, i);
			tk->cmd(t, "frame " + padf + " -width " + string ENDGAP);
			tk->cmd(t, "pack " + padf + " -side left");
		}
		i++;
	}
	...
	# pack rows in frame
	for(j = 0; j < nrow; j++)
		tk->cmd(t, sys->sprint("pack .f%d .pad%d -fill x -in .", j, j));

	(w, h) := (int tk->cmd(t, ". cget -width"), int tk->cmd(t, ". cget -height"));
	r := ctxt.screen.image.r;
	off := (r.dx()-w)/2;
	tk->cmd(t, sys->sprint(". configure -x %d -y %d", r.min.x+off, r.max.y-h));

	spawn handle_keyclicks(t, ctxt.screen, wcmd, taskbar);
}
...
handle_keyclicks(t: ref Tk->Toplevel, screen: ref Draw->Screen, wcmd: chan of string, taskbar: int)
{
				...
				if(collecting){
					...
					if(c == -1){
						for(i := 0; i < len collected; i++)
							tk->keyboard(screen, collected[i]);
						continue;
					}
				}
...
active(t: ref Tk->Toplevel, keyno: int, on: int)
{
	...
	for(xl := key[0].x; xl != nil; xl = tl xl){
		...
		if(key[0].on)
			col = "white";
		tk->cmd(t, ".b"+string hd xl+" configure -bg "+col);
	}
}
...
redraw(t: ref Tk->Toplevel)
{
	...
	for(i:=0; i<len bank; i++) {
		...
		if(key != nil){
			...
			tk->cmd(t, ".b" + string i + " configure -text {" + key + "} -command 'send keypress " + string val);
		}
  	}
	tk->cmd(t, "update");
}
