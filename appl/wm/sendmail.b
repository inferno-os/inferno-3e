...
init(xctxt: ref Context, argv: list of string)
{
	...
	(main, titlectl) = wmlib->titlebar(ctxt.screen, tkargs,
				"MailStop: Sender", Wmlib->Appl);
...
{
	if(srv.dfd == nil) {
		wmlib->dialog(main, "error -fg red", "Send",
	...
	if(tk->cmd(main, ".body.t index end") == "1.0") {
		...
		if(wmlib->dialog(main, "warning -fg yellow", "Send",
...
save()
{
	...
	for(;;) {
		fname = wmlib->filename(ctxt.screen, main, "Save in Mailbox", pat,
