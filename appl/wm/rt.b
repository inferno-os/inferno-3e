...
init(ctxt: ref Draw->Context, nil: list of string)
{
	...
	(t, menubut) = wmlib->titlebar(ctxt.screen, "", "Dis Module Manager", Wmlib->Appl);
...
writedis()
{
	...
	while((fd := sys->open(disfile, Sys->OREAD)) == nil){
		if(wmlib->dialog(t, "error -fg red", "Open Dis File", "open failed: "+sprint("%r"),
		     0, "Retry" :: "Abort" :: nil))
			return;
	}
	...
	disbuf := array[d.length] of byte;
	if(sys->read(fd, disbuf, d.length) != d.length){
		ioerror("Reading Dis file "+disfile, "read error: "+sprint("%r"));
		return;
	}
	outbuf := array[d.length+2*4] of byte;	# could avoid this buffer if required, by writing portions of disbuf
...
{
	wmlib->dialog(t, "error -fg red", title, err, 0, "Dismiss" :: nil);
}
