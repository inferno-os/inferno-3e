...
loadsrc(src: string, addpath: int): ref Mod
{
	...
	if(ml == nil || m.tk == nil){
		...
		m.tk = ".t."+wmlib->tkquote(file)+string srcid++;
		if(xscroll)
...
attachdis(m: ref Mod): int
{
	...
	if(c == nil){
		...
		m.dis = wmlib->filename(screen, tktop, "Locate Dis file", pat, dir);
		c = load Diss m.dis;
	}
	return c != nil;
}
...
attachsym(m: ref Mod)
{
	...
	sbl = wmlib->filename(screen, tktop, "Locate Symbol file", pat, dir);
	tk->cmd(tktop, "cursor -bitmap cursor.wait");
	(m.sym, err) = debug->sym(sbl);
	tk->cmd(tktop, "cursor -default");
	if(m.sym != nil)
...
search(s: string): int
{
	...
	if(len sel > 0 && sel[0] == '!')
		pos = " insert";
	pos = tk->cmd(tktop, packed.tk+" search -- "+wmlib->tkquote(s)+pos);
...
findsrc(src: string): ref Mod
{
	...
	src = wmlib->filename(screen, tktop, "Locate Limbo Source", pat, opdir);
	if(src == nil)
		return nil;
