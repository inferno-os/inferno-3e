...
init(ctxt: ref Context, nil: list of string)
{
	...
	(t, menubut) = wmlib->titlebar(screen, "", "Infernal Coffee", 0);
...
frameposn(t: ref Toplevel): Rect
{
	r: Rect;
	r.min.x = int tk->cmd(t, ".f cget -actx") + int tk->cmd(t, ".f cget -bd");
	r.min.y = int tk->cmd(t, ".f cget -acty") + int tk->cmd(t, ".f cget -bd");
	r.max.x = r.min.x + int tk->cmd(t, ".f cget -actwidth");
	r.max.y = r.min.y + int tk->cmd(t, ".f cget -actheight");
	return r;
}
...
animate(r: Rect, ctl: chan of (string, int, int))
{
	...
	for(k:=0; ; k++){
		...
		hell.draw((org, (org.x+buffer.r.dx(), org.y+buffer.r.dy())), buffer, nil, buffer.r.min);
		sys->sleep(5);
		alt{
		(cmd, i0, i1) := <-ctl =>
	Pause:
