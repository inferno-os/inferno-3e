...
loop:	for(; argv != nil; argv = tl argv){
	...
	(tktop, titlebut) = wmlib->titlebar(screen, nil, title, Wmlib->Appl);
	...
	debsrc->init(screen, tktop, sys, tk, wmlib, str, debug, opts.xscroll, opts.remcr);
	(datactl, datatitle) := debdata->init(screen, wmlib->geom(tktop), debsrc, sys, tk, str, debug);
