...
init(xctxt: ref Draw->Context, nil: list of string)
{
	...
	wmlib->init();
	(t, menubut) := wmlib->titlebar(ctxt.screen, "", "AVI Player", 0);
	...
	press := <-cmd =>
		...
		"file" =>
			...
			fname = wmlib->filename(ctxt.screen, t, "Locate AVI files",
				patterns, nil);
			if(fname != nil) {
...
play(t: ref Toplevel, file: string)
{
	...
	(r, err) := avi->open(file);
	if(err != nil) {
		wmlib->dialog(t, "error -fg red", "Open AVI file", err, 0, sp);
		return;
	}

	err = avi->r.check4("AVI ");
	if(err != nil) {
		wmlib->dialog(t, "error -fg red", "Read AVI format", err, 0, sp);
		return;
	}

	(code, l) := avi->r.gethdr();
	if(code != "LIST") {
		wmlib->dialog(t, "error -fg red", "Parse AVI headers",
	...
	if(err != nil) {
		wmlib->dialog(t, "error -fg red", "Read AVI header", err, 0, sp);
		return;
	}
