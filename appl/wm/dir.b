...
init(env: ref Draw->Context, argv: list of string)
{
	...
	(t, titlebut) := wmlib->titlebar(ctxt.screen, "", "", Wmlib->Appl);
...
button1(t: ref Toplevel, item: ref Dir)
{
	...
	if(item.mode & sys->CHDIR) {
		...
		defcursor(t);
		if(mod == nil) {
			wmlib->dialog(t, "error -fg red", "Load Dir module",
				sys->sprint("Error: %r"),
				0, "Continue"::nil);
			return;
		}
...
button2(t: ref Toplevel, stat: ref Sys->Dir)
{
	ft := filetype(t, stat, stat.name);
	img := sys->sprint("-file %s.bit -maskfile %s.mask", ft.icon, ft.icon);
