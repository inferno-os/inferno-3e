...
tkcmdlist := array[] of {
	...
	"bind .m2 <ButtonRelease> {.m2 tkMenuButtonUp %x %y}",
	"bind .w.t <ButtonPress-3> {.m3 post %x %y; grab set .m3}",
	"bind .m3 <ButtonRelease> {.m3 tkMenuButtonUp %x %y}",
...
newflayer(tag, tp: int): ref Flayer
{
	...
	(t, cmdc) := wmlib->titlebar(ctxt.ctxt.screen, "-borderwidth 1 -relief raised", "SamTerm", Wmlib->Appl);
	...
	if (tp) {
		lines = 8;
		wmlib->tkcmds(t, tksam1);
		mkmenu2c(t);
	} else {
		lines = 20;
		wmlib->tkcmds(t, tkwork1);
		mkmenu2(t);
	}
	mkmenu3(t);
	wmlib->tkcmds(t, tkcmdlist);
...
{
	wmlib->titlectl(ctxt.flayers[win].t, menu);
}
