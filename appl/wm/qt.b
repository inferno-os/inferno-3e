...
init(xctxt: ref Draw->Context, nil: list of string)
{
	...
	wmlib->init();
	(t, menubut) := wmlib->titlebar(ctxt.screen, "", "QuickTime Player", 0);
	...
	press := <-cmd =>
		...
		"file" =>
			...
			fname = wmlib->filename(ctxt.screen, t, "Locate Movie", pat, "");
			if(fname != nil) {
				s := fname;
				if(len s > 25)
					s = "..."+fname[len s - 25:];
	...
	for(;;) {
		...
		* =>
		...
		"mvhd" =>
			...
			if(err == nil)
				break;
			wmlib->dialog(t, "error -fg red", "Parse Headers",
					err,
					0, "Exit"::nil);
			exit;
		"trak" =>
			...
			if(err == nil)
				break;
			wmlib->dialog(t, "error -fg red", "Parse Track",
					err,
					0, "Exit"::nil);
			exit;
		}
...
play(t: ref Toplevel, file: string)
{
	(q, err) := qt->open(file);
	if(err != nil) {
		wmlib->dialog(t, "error -fg red", "Open Movie",
					"Failed to open \""+file+"\"\n"+err,
					0, "Continue"::nil);
		return;
	}
