...
init(xctxt: ref Context, argv: list of string)
{
	...
	(main, titlectl) = wmlib->titlebar(ctxt.screen, tkargs, "Readmail: Reader", Wmlib->Appl);

	msg := chan of string;
	tk->namechan(main, msg, "msg");
	hdr := chan of string;

	wmlib->tkcmds(main, mail_cfg);
...
{
	(hdr, hdrctl) := wmlib->titlebar(ctxt.screen, wmlib->geom(main),
				"Readmail: Headers", Wmlib->Appl);

	tk->namechan(hdr, tomain, "tomain");

	wmlib->tkcmds(hdr, hdr_cfg);
...
topit(hdr: ref Toplevel, msg: int): int
{
	...
	if(err != nil) {
		wmlib->dialog(hdr, "error -fg red", "POP3 Error",
...
{
	if(cmesg == 0) {
		wmlib->dialog(main, "error -fg red", "Reply",
...
{
	if(cmesg == 0) {
		wmlib->dialog(main, "error -fg red", "Save",
				"No current message",
				0, "Continue"::nil);
		return;
	}
	...
	fname: string;
	for(;;) {
		fname = wmlib->filename(ctxt.screen, main, "Save in Mailbox",
		...
		r := wmlib->dialog(main, "error -fg red", "Save",
...
{
	if(srv.dfd == nil) {
		wmlib->dialog(main, "warning -fg yellow", "Delete",
	...
	if(err != nil) {
		wmlib->dialog(main, "error -fg red", "Delete",
...
disconnect()
{
	...
	if(err != nil) {
		wmlib->dialog(main, "error -fg red", "Disconnect",
...
{
	(t, conctl) := wmlib->titlebar(ctxt.screen, postposn(parent),
				"Connection Parameters", 0);

	cmd := chan of string;
	tk->namechan(t, cmd, "cmd");

	wmlib->tkcmds(t, con_cfg);
	...
	s = <-cmd =>
		...
		if(server == "") {
			wmlib->dialog(t, "error -fg red", "Connect",
...
initialize()
{
	(err, s) := pop3cmd("STAT");
	if(err != nil) {
		wmlib->dialog(main, "error -fg red", "Mailbox Status",
...
{
	if(srv.dfd == nil) {
		wmlib->dialog(main, "warning -fg yellow", "Read",
	...
	if(err != nil) {
		wmlib->dialog(main, "error -fg red", "Read",
...
dialer(t: ref Toplevel, server, user, pass: string): int
{
	...
	for(;;) {
		...
		if(ok >= 0)
			...
			ok = wmlib->dialog(t, "error -fg", "Connect",
					"The following error occurred while\n"+
