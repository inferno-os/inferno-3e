...
init(ctxt: ref Draw->Context, argv: list of string)
{
	...
	(win, winctl) := wmlib->titlebar(ctxt.screen, "", "Tetris",Wmlib->Appl);
	seedrand();
	...
	if (ctxt.screen.image != nil) {
		img := ctxt.screen.image;
		wsz := wsize(win, ".");
...
tablerow(win: ref Tk->Toplevel, w, bg: string, relief: string, vals: array of string, widths: array of string)
{
	...
	for (i := 0; i < len vals; i++) {
		cw := cmd(win, "label " + w + "." + string i + " -text " + wmlib->tkquote(vals[i]) + " -width " + widths[i] + bg);
...
{
	cmd(top, "frame " + w);
	cmd(top, "label " + w + ".title -text " + wmlib->tkquote(title));
	cmd(top, "label " + w +
