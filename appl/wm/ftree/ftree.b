...
init(ctxt: ref Draw->Context, argv: list of string)
{
	loadmods();
	noexit := 0;
	winopts := Wmlib->Resize | Wmlib->Hide;
	arg->init(argv);
	while ((opt := arg->opt()) != 0) {
		case opt {
		'e' =>
			(noexit, winopts) = (1, Wmlib->Resize);
		'E' =>
			(noexit, winopts) = (1, 0);
		'p' =>
			(noexit, winopts) = (0, 0);
	...
	if (argv != nil) {
		...
		if (ok == -1) {
			...
			sys->raise("fail:bad root");
		} else if ((s.mode & Sys->CHDIR) == 0) {
	...
	(win, wmctl) := wmlib->titlebar(ctxt.screen, nil, "Ftree", winopts);
	tkwin = win;
	...
	if (sh->run(ctxt, "lib/plumber" :: "-w" :: "-c/chan/plumbstart" :: nil) == nil) {
		plumbmsg = load Plumbmsg Plumbmsg->PATH;
		if (plumbmsg->init(1, nil, 0) == -1) {
...
{
	if ((ref Msg("browser", nil, nil, "text", nil, array of byte f)).send() == -1) {
...
loadmods()
{
	...
	(rc, nil) := sys->stat("#e");
	if(rc < 0)
		sys->bind("/dev/null", "/dis/lib/env.dis", Sys->MREPL);
	sh = load Sh Sh->PATH;
	if (sh == nil)
		badmodule(Sh->PATH);
...
expand(t: ref Tree.N, it: Item)
{
	...
	for (i := 0; i < len d; i++) {
		...
		(t.sub[i], t.e.children[i]) = makenode(d[i].mode & Sys->CHDIR, d[i].name, tagname);
