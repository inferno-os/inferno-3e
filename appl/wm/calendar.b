...
init(ctxt: ref Draw->Context, argv: list of string)
{
	...
	if (argv != nil)
		scheddir = hd argv;
	(top, wmctl) := wmlib->titlebar(ctxt.screen, "", X("Calendar"), Wmlib->Appl);
	if (top == nil) {
		sys->fprint(stderr, "cal: cannot make window: %r\n");
		sys->raise("fail:cannot make window");
	}
	...
	if (currtime == nil) {
		...
		sys->raise("fail:failed to get local time");
	}
	...
	for (;;) {
		...
		c := <-cmdch =>
			...
			"save" =>
				if (dirty && save(top, cal, ms, date) != -1)
					dirty = 0;
			"settime" =>
				raisesettime(top);
			"showtime" =>
		...
		c := <-wmctl =>
			if (c == "exit" && dirty)
				save(top, cal, ms, date);
			wmlib->titlectl(top, c);
		}
		if (ndate != date) {
			e := 0;
			if (dirty)
				e = save(top, cal, ms, date);
			if (e != -1) {
				dirty = 0;
...
save(top: ref Tk->Toplevel, cal: ref Cal, ms: ref Markset, date: int): int
{
	...
	if (ok == -1) {
		notice(top, "Cannot save entry: " + err);
		return -1;
	}
	return 0;
}

notice(top: ref Tk->Toplevel, s: string)
{
	wmlib->dialog(top, nil, "Notice", s, 0, "OK"::nil);
}
...
{
	f := Font.open(top.image.display, font);
	if (f == nil) {
...
raisesettime(top: ref Tk->Toplevel)
{
	panelcmds := array[] of {
	"frame .d",
	...
	if (fd == nil) {
		notice(top, X("Cannot set time: ") + sys->sprint("%r"));
		return;
	}
	(panel, wmctl) := wmlib->titlebar(top.image.screen, wmlib->geom(top),
					X("Set Time"), 0);
	...
	# wouldn't work with current wmlib->dialog() either...
	cmd(panel, "update");
loop: for (;;) alt {
	c := <-cmdch =>
		case c {
		"set" =>
			...
			if (err == nil)
				break loop;
			notice(panel, X("Cannot set time: ") + err);
		"cancel" =>
			break loop;
		* =>;
		}
