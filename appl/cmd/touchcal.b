...
tkconfig(w, h: int): array of string
{
	return array[] of {
	"frame .c -borderwidth 0 -bg white -width "+string w+" -height "+string h,
	"bind .c <Button-1> {send cmd push %x %y}",
	"bind .c <Motion-Button-1> {send cmd move %x %y}",
	"bind .c <ButtonRelease-1> {send cmd release %x %y}",
...
init(ctxt: ref Draw->Context, args: list of string)
{
	...
	if(ctxt != nil && ctxt.screen != nil){
		# under Tk, allocate a screen-sized frame to catch mouse events
		tk = load Tk Tk->PATH;
		if(tk == nil)
			err(sys->sprint("no Tk module: %r"));
		disp = ctxt.screen.image;
		r = disp.r;
		top = tk->toplevel(ctxt.screen, sys->sprint("-width %d -height %d -x %d -y %d", r.dx(), r.dy(), r.min.x, r.min.y));
		tkf := tkconfig(r.dx(), r.dy());
		for(i := 0; i < len tkf; i++)
			tk->cmd(top, tkf[i]);
		cmd := chan of string;
		tk->namechan(top, cmd, "cmd");
		spawn tkmouse(cmd, ptr, pidc);
	}else{
		# standalone, catch them ourselves
		mfd := sys->open("/dev/pointer", Sys->OREAD);
		if(mfd == nil)
			err(sys->sprint("can't open /dev/pointer: %r"));
		spawn mouse(mfd, ptr, pidc);
	}
	mousepid = <-pidc;
	...
	disp.draw(r, white, display.ones, display.ones.r.min);
...
mouse(fd: ref Sys->FD, mc: chan of ref Pointer, pidc: chan of int)
{
	pidc <-= sys->pctl(0, nil);
	...
	for(;;){
		...
		mc <-= ref Pointer(b, (x,y));
	}
}

tkmouse(cmd: chan of string, mc: chan of ref Pointer, pidc: chan of int)
{
	pidc <-= sys->pctl(0, nil);
	m := Pointer(0, (0,0));
	for(;;){
		...
		if(nf < 3)
			continue;
		m.xy = (int hd tl flds, int hd tl tl flds);
		case hd flds {
		"push" =>
			m.buttons = 1;
		"release" =>
