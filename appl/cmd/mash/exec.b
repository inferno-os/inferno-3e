...
#
waitfor(e: ref Env, pid: int, wc: chan of int, ec, xc: chan of ref Sys->Exception)
{
	if (ec != nil || xc != nil) {
		spawn waiter(e, pid, wc);
		if (ec == nil)
			ec = chan of ref Sys->Exception;
		if (xc == nil)
			xc = chan of ref Sys->Exception;
		alt {
		<-wc =>
			return;
		x := <-ec =>
...
#
mkprog(args: list of string, e: ref Env, in, out: ref Sys->FD, wc: chan of int, ec, xc: chan of ref Sys->Exception)
{
	...
	x := ref Sys->Exception;
	cmd := hd args;
	if (sys->rescue(FAILPAT, x) == Sys->EXCEPTION) {
		if (xc != nil)
			xc <-= x;
	...
	if (sys->rescue(EPIPE, x) == Sys->EXCEPTION) {
		if (xc != nil)
			xc <-= x;
		#else
		#	sys->fprint(console, "%s: %s\n", cmd, x.name);
	...
	if (sys->rescue(EXIT, x) == Sys->EXCEPTION) {
		if (ec != nil)
			ec <-= x;
		exit;
	}
