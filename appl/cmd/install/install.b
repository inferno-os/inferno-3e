...
init(nil: ref Draw->Context, args: list of string)
{
	...
	for (dirix := 0; dirix < len reqdirs; dirix++) {
		...
		if (exists == -1) {
			fd := sys->create(dir, Sys->OREAD, Sys->CHDIR + 8r7775);
			if (fd == nil)
	...
	else if (sys->create(MTPT+root, Sys->OREAD, 8r775 | Sys->CHDIR) == nil)
	...
	else if (sys->create(MTPT+root+"/tmp", Sys->OREAD, 8r775 | Sys->CHDIR) == nil)
