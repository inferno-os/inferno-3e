...
# fd = nil;
	...
	(a.d.length, nil) = str->toint(hd fs, 10);	fs = tl fs;
...
puthdr(b : ref Iobuf, name : string, d : ref Sys->Dir)
{
	...
	if(addp){
		mode |= 8r664;
		if(mode & Sys->CHDIR || mode & 8r111)
			mode |= 8r111;
	}
	b.puts(sys->sprint("%s %uo %s %s %ud %d\n", name, mode, d.uid, d.gid, d.mtime, d.length));
}
