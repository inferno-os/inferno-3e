...
openupdate(d : string) : (array of Update, int, int)
{
	...
	while (sys->dirread(fd, dir) > 0) {
		...
		u[nu].typ = k;
		if (readupdate(u, nu, d, dir[0].name) != nil)
			nu++;
	}
	if (nu == 0)
		return (nil, -1, 0);
...
readfile(s : string) : string
{
	...
	if ((fd := sys->open(s, Sys->OREAD)) == nil || (n := sys->read(fd, buf, d.length)) != d.length)
		return nil;
	s = string buf;
	ls := len s;
	if (s[ls-1] == '\n')
...
bsearch(b : ref Bufio->Iobuf, p : string) : string
{
	if (b == nil)
		return nil;
	lo := 0;
	b.seek(0, Bufio->SEEKEND);
	hi := b.offset();
	l := len p;
	while (lo < hi) {
		m := (lo+hi)/2;
		b.seek(m, Bufio->SEEKSTART);
		b.gets('\n');
		if (b.offset() == hi) {
			bgetbackc(b);
			m = b.offset();
