...
init(nil: ref Draw->Context, args: list of string)
{
	...
	while ((a := arch->gethdr(ar)) != nil) {
		...
		if(!docopy){
			wrap->md5sum(ar.b, digest0, a.d.length);
		...
		if(hflag){
			bout.puts(sys->sprint("%s %uo %s %s %ud %d\n",
				name, a.d.mode, a.d.uid, a.d.gid, a.d.mtime, a.d.length));
			arch->drain(ar, a.d.length);
			continue;
		}
		if(a.d.mode & Sys->CHDIR)
			mkdir(name, a.d);
		else
			extract(ar, name, a.d);
	}
...
doremove(s : string)
{
	...
	if(wrap->md5file(p, digest) < 0)
		...
		(ok, t) := wrap->getfileinfo(oldw, s, digest, nil, digest1);
		if(force == 0 && ok < 0)
...
mkdirs(basedir, name: string)
{
	...
	while(names != nil) {
		create(basedir, Sys->OREAD, 8r775|Sys->CHDIR);
		if(tl names == nil)
			break;
...
extract(ar : ref Arch->Archive, name: string, dir : ref Sys->Dir)
{
	...
	err := arch->getfile(ar, b, dir.length);
	if (err != nil) {
