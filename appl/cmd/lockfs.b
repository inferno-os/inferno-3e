...
include "styxlib.m";
	styx: Styxlib;
	Dirtab, Styxserver, Tmsg, Rmsg, Chan,
	devdir,
	Eperm, Ebadfid, Eexists, Enotdir, Enotfound: import styx;
include "arg.m";
include "keyring.m";
	keyring: Keyring;
include "security.m";
...
init(nil: ref Draw->Context, argv: list of string)
{
	...
	if (addr != nil) {
		...
		rq <-= (fds[0], "lock", nil);
		rq <-= (nil, nil, nil);
		if (sys->mount(fds[1], mountpoint, Sys->MREPL | Sys->MCREATE, nil) == -1) {
			sys->fprint(stderr, "lockfs: cannot mount: %r\n");
			sys->raise("fail:cannot mount");
		}
	}
}
...
dirgen(nil: ref Styxserver, c: ref Styxlib->Chan,
...
{
	e := ref Sys->Exception;
	if (sys->rescue("panic:*", e) == Sys->EXCEPTION) {
		sys->rescued(Sys->ONCE, nil);
		sys->fprint(stderr, "lockfs: %s\n", e.name);
		srv.reply(ref Rmsg.Error(gm.tag, e.name[len "panic:":]));
		return 0;
	}
	if (gm == nil)
	...
	Create =>
		...
		c.open = 1;
		srv.reply(ref Rmsg.Create(m.tag, m.fid, c.qid));
	Read =>
		c := fid2chan(srv, m.fid);
		if (c.qid.path & Sys->CHDIR)
			...
			if (l == nil)
				...
				sys->seek(l.fd, int m.offset, Sys->SEEKSTART);
				n := sys->read(l.fd, d, m.count);
				if (n == -1)
					srv.reply(ref Rmsg.Error(m.tag, sys->sprint("%r")));
				else {
					srv.reply(ref Rmsg.Read(m.tag, m.fid, d[0:n]));
					l.d.atime = now();
				}
			}
		}
	Write =>
		...
		if (n == -1)
			...
			srv.reply(ref Rmsg.Write(m.tag, m.fid, n));
			nlength := int m.offset + n;
	...
	Wstat =>
		...
		d.qid = l.d.qid;
		l.d = d;
		srv.reply(ref Rmsg.Wstat(m.tag, m.fid));
	Attach =>
		srv.devattach(m);
	}
	return 0;
...
openlockfile(l: ref Lockfile, req: ref Openreq): int
{
	...
	if (req.omode & Sys->OTRUNC)
		l.d.length = 0;
	req.srv.reply(ref Rmsg.Open(req.tag, req.c.fid, l.d.qid));
...
lockinit()
{
	...
	for (locks = nil; lockl != nil; lockl = tl lockl)
		if (((hd lockl).d.mode & Sys->CHDIR) == 0)
