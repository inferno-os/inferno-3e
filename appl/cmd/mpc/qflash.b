...
# could come from file or #F/flashstat
...
init(nil: ref Draw->Context, args: list of string)
{
	...
	if(dir.length > avail)
		err(sys->sprint("%s contents %ud bytes, exceeds flash region %ud bytes", fname, dir.length, avail));
	...
	if(save){
		...
		saved = saveflash(flash, where+dir.length, region.limit) :: saved;
	}
