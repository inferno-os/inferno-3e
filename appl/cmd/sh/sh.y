...
%token DUP REDIR WORD OP END ERROR

%type <node> redir word nlsimple simple cmd shell assign
%type <node> cmdsan cmdsa pipe comword line body list
...
cmdsa: 	pipe ';'		{$$ = $pipe; }
	| pipe '&'			{$$ = mk(n_NOWAIT, $pipe, nil, nil, nil); }
cmdsan:	cmdsa
	| pipe '\n'			{$$ = $pipe; }
pipe:					{$$ = nil;}
	| cmd
	| pipe '|' optnl cmd	{$$ = mk(n_PIPE, $pipe, $cmd, nil, $2); }
cmd:	simple
	| redir cmd		{$$ = mk(n_ADJ, $redir, $cmd, nil, nil); }
	| redir
	| assign
assign: word '=' assign	{$$ = mk($2, $word, $assign, nil, nil); }
	| word '=' simple	{$$ = mk($2, $word, $simple, nil, nil); }
	| word '='			{$$ = mk($2, $word, nil, nil, nil); }
redir:	DUP			{$$ = mk(n_DUP, nil, nil, nil, $DUP); }
	| REDIR word		{$$ = mk(n_REDIR, $word, nil, nil, $REDIR); }
simple:	word
	| simple word		{$$ = mk(n_ADJ, $simple, $word, nil, nil); }
	| simple redir		{$$ = mk(n_ADJ, $simple, $redir, nil, nil); }
list:	optnl			{$$ = nil;}
	| nlsimple optnl
nlsimple: optnl word		{$$ = $word; }
	| nlsimple optnl word	{$$ = mk(n_ADJ, $nlsimple, $word, nil, nil); }
	| nlsimple optnl redir  {$$ = mk(n_ADJ, $nlsimple, $redir, nil, nil); }
word:	comword
	| word '^' optnl comword	{$$ = mk(n_CONCAT, $word, $comword, nil, nil); }
comword: WORD		{$$ = mk(n_WORD, nil, nil, $WORD, nil); }
	| OP comword		{$$ = mk($OP, $comword, nil, nil, nil); }
	| '(' list ')'			{$$ = mk(n_LIST, $list, nil, nil, nil); }
	| '{' body '}'		{$$ = mk(n_BLOCK, $body, nil, nil, nil); }
optnl:  # null
	| optnl '\n'
%%
