...
builtin_rescue(ctxt: ref Context, args: list of ref Listnode, last: int) : string
{
	...
	if (sys->rescue("fail:" + (hd args).word, e) == Sys->EXCEPTION) {
		sys->rescued(Sys->ONCE, nil);
		ctxt.push();
		ctxt.set("exception", ref Listnode(nil, e.name[5:]) :: nil);
...
sbuiltin_pipe(ctxt: ref Context, argv: list of ref Listnode): list of ref Listnode
{
	...
	* =>
	...
	if (<-sync == -1)
		ctxt.fail("bad pipe", "pipe: cannot open command pipe");
	return ref Listnode(nil, TMPDIR + "/" + id + "y") :: nil;
}

mkdir(f: string): int
{
	if (sys->create(f, Sys->OREAD, Sys->CHDIR | 8r777) == nil)
		return -1;
	return 0;
}

runpipe(sync: chan of int, ctxt: ref Context, fdno: int, p: string, cmd: ref Listnode)
{
...
xraise(s: string)
{
	...
	if (len d > Sys->WAITLEN)
		sys->raise(string d[0:Sys->WAITLEN]);
	else {
		d = nil;
		sys->raise(s);
	}
