...
GLOB: con 1;				# thanks to plan 9's rc
...
runfile(ctxt: ref Context, fd: ref Sys->FD, path: string, args: list of ref Listnode)
{
	...
	while (!lex.eof) {
		...
		if (err != nil) {
			...
			if (!interactive)
				sys->raise("fail:parse error");
		} else if (n != nil) {
			if (interactive) {
				e2 := ref Sys->Exception;
				if (sys->rescue("fail:*", e2) == Sys->EXCEPTION) {
					sys->rescued(Sys->ONCE, nil);
					laststatus = e2.name[5:];
				} else {
...
pipe2cmd(n: ref Node): ref Node
{
	if (n == nil || n.ntype != n_PIPE)
		return n;
	return mk(n_ADJ, mk(n_BLOCK,n,nil,nil,nil), mk(n_VAR,mk(n_WORD,nil,nil,"*",nil),nil,nil,nil), nil,nil);
}
...
walk(ctxt: ref Context, n: ref Node, last: int): string
{
	...
	* =>
		...
		if (bg) {
			...
			if (ctxt.options() & Context.INTERACTIVE)
				sys->fprint(stderr(), "%d\n", pid);
		...
		startchan: chan of (int, ref Expropagate))
{
	...
	if (status != nil) {
		sys->unrescue();			# don't propagate bad status as an exception.
		xraise("fail:" + status);
	}
}
...
runexternal(ctxt: ref Context, args: list of ref Listnode, last: int): string
{
	...
	do {
		...
		if (mod != nil) {
			...
			if (last) {
				if(sys->rescue(EPIPE, nil) == Sys->EXCEPTION) {
					sys->rescued(Sys->ONCE, nil);
					return EPIPE;
				}
				e := ref Sys->Exception;
	...
	} while (pathlist != nil && err == ENOEXIST);
		...
		drawcontext: ref Draw->Context, argv: list of string, startchan: chan of int, keepfds: list of int)
{
	...
	if(sys->rescue(EPIPE, nil) == Sys->EXCEPTION)
		sys->rescued(Sys->RAISE, "fail:" + EPIPE);
...
doredirs(ctxt: ref Context, redirs: ref Redirlist): list of int
{
	...
	for (; rl != nil; rl = tl rl) {
		...
		if (path == nil && rfd == nil) {
			# dup
			if (fd1 == -1 || fd2 == -1)
				ctxt.fail("bad redir", "invalid dup");
		...
		if (rfd == nil) {
			...
			if (append)
				sys->seek(rfd, 0, Sys->SEEKEND);	# not good enough, but alright for some purposes.
		}
...
{
	names := array[] of {"load", "unload", "loaded", "builtin", "whatis", "run", "exit", "@"};
...
runsbuiltin(ctxt: ref Context, nil: Sh, argv: list of ref Listnode): list of ref Listnode
{
	...
	"quote" =>	return sbuiltin_quote(ctxt, argv);
...
sbuiltin_quote(nil: ref Context, argv: list of ref Listnode): list of ref Listnode
{
	return ref Listnode(nil, quoted(tl argv, 1)) :: nil;
}
...
{
	(ok, info) := s;
	return ok != -1 && (info.mode & Sys->CHDIR) == 0
			&& (info.mode & mode) != 0;
}
...
mkseq(left, right: ref Node): ref Node
{
	if (left != nil && right != nil)
		return mk(n_SEQ, left, right, nil, nil);
	else if (left == nil)
		return right;
	return left;
}

mk(ntype: int, left, right: ref Node, word: string, redir: ref Redir): ref Node
{
	return ref Node(ntype, left, right, word, redir);
}
...
{
	d := array of byte s;
	if (len d > Sys->WAITLEN - 1)
		sys->raise(string d[0:Sys->WAITLEN - 1]);
	else {
		d = nil;
		sys->raise(s);
	}
...
YYLAST:	con 92;
yyact := array[] of {
  10,   8,  13,   4,   9,  34,   6,   7,  26,  27,
  48,   5,  36,  31,  44,  28,  14,  15,  32,  29,
  30,  37,  46,  30,  33,  38,  16,  19,  17,  39,
  40,  20,   2,   1,  41,  45,  43,  42,  25,  35,
   0,  47,  37,  26,  27,   0,  52,  51,  53,  54,
  39,  50,  11,  12,  14,  15,   0,   0,   0,  24,
  46,  49,  22,  23,  16,   3,  17,  11,  12,  14,
  15,  11,  12,  14,  15,   0,  14,  15,  18,  16,
   0,  17,  21,  16,   0,  17,  16,  24,  17,   0,
  22,  23,
};
yypact := array[] of {
  63,-1000,  19,  19,  77,  67,-1000,  67,  67,-1000,
   8,-1000,  70,-1000,-1000,  70,-1000,  67,-1000,-1000,
-1000,-1000,-1000,-1000,-1000,-1000,   5,-1000,-1000,  70,
-1000,   5,-1000,  -3,  10,-1000,  -9,  49,  67,-1000,
  48,-1000,  67,  10,-1000,   5,-1000,  48,-1000,-1000,
-1000,-1000,-1000,   5,-1000,
};
yypgo := array[] of {
   0,   1,   0,  39,   7,   6,  33,   4,  25,  11,
   3,   2,  32,  12,  24,  78,   5,
};
yyr1 := array[] of {
   0,   6,   6,  15,  15,  12,  12,  13,  13,   9,
   9,   8,   8,  10,  10,  10,   5,   5,   5,   5,
   7,   7,   7,   1,   1,   4,   4,   4,  14,  14,
   3,   3,   3,   2,   2,  11,  11,  11,  11,  16,
  16,
};
yyr2 := array[] of {
   0,   2,   2,   1,   1,   1,   2,   1,   2,   2,
...
yychk := array[] of {
-1000,  -6, -12,   2, -10,  -9,  -5,  -4,  -1,  -7,
  -2,   4,   5, -11,   6,   7,  16,  18, -15,   8,
  12, -15,  13,  14,  10, -12,  -2,  -1,  -5,  11,
  15,  -2, -11, -14, -16,  -3, -13, -10,  -8,  -9,
 -16,  -7,  -4, -16,  17,  -2,  12, -16,  19,  12,
 -13,  -5, -11,  -2,  -1,
};
yydef := array[] of {
  -2,  -2,   0,   0,   5,  13,  14,  16,  18,  19,
  25,  23,   0,  33,  35,   0,  39,  13,   1,   3,
   4,   2,   9,  10,  39,   6,  26,  27,  17,  22,
  39,  24,  36,   0,  28,  39,   0,   7,  13,  11,
   0,  20,  21,   0,  37,  30,  40,  29,  38,  12,
   8,  15,  34,  31,  32,
};
yytok1 := array[] of {
   1,   3,   3,   3,   3,   3,   3,   3,   3,   3,
...
10=>
{yyval.node = mk(n_NOWAIT, yys[yypt-1].yyv.node, nil, nil, nil); }
11=>
yyval.node = yys[yyp+1].yyv.node;
12=>
{yyval.node = yys[yypt-1].yyv.node; }
...
yyval.node = yys[yyp+1].yyv.node;
15=>
{yyval.node = mk(n_PIPE, yys[yypt-3].yyv.node, yys[yypt-0].yyv.node, nil, yys[yypt-2].yyv.redir); }
16=>
yyval.node = yys[yyp+1].yyv.node;
17=>
{yyval.node = mk(n_ADJ, yys[yypt-1].yyv.node, yys[yypt-0].yyv.node, nil, nil); }
18=>
yyval.node = yys[yyp+1].yyv.node;
19=>
yyval.node = yys[yyp+1].yyv.node;
20=>
{yyval.node = mk(yys[yypt-1].yyv.optype, yys[yypt-2].yyv.node, yys[yypt-0].yyv.node, nil, nil); }
21=>
{yyval.node = mk(yys[yypt-1].yyv.optype, yys[yypt-2].yyv.node, yys[yypt-0].yyv.node, nil, nil); }
22=>
{yyval.node = mk(yys[yypt-0].yyv.optype, yys[yypt-1].yyv.node, nil, nil, nil); }
23=>
{yyval.node = mk(n_DUP, nil, nil, nil, yys[yypt-0].yyv.redir); }
24=>
{yyval.node = mk(n_REDIR, yys[yypt-0].yyv.node, nil, nil, yys[yypt-1].yyv.redir); }
25=>
yyval.node = yys[yyp+1].yyv.node;
26=>
{yyval.node = mk(n_ADJ, yys[yypt-1].yyv.node, yys[yypt-0].yyv.node, nil, nil); }
...
31=>
{yyval.node = mk(n_ADJ, yys[yypt-2].yyv.node, yys[yypt-0].yyv.node, nil, nil); }
32=>
{yyval.node = mk(n_ADJ, yys[yypt-2].yyv.node, yys[yypt-0].yyv.node, nil, nil); }
33=>
yyval.node = yys[yyp+1].yyv.node;
34=>
{yyval.node = mk(n_CONCAT, yys[yypt-3].yyv.node, yys[yypt-0].yyv.node, nil, nil); }
35=>
{yyval.node = mk(n_WORD, nil, nil, yys[yypt-0].yyv.word, nil); }
36=>
{yyval.node = mk(yys[yypt-1].yyv.optype, yys[yypt-0].yyv.node, nil, nil, nil); }
37=>
{yyval.node = mk(n_LIST, yys[yypt-1].yyv.node, nil, nil, nil); }
38=>
{yyval.node = mk(n_BLOCK, yys[yypt-1].yyv.node, nil, nil, nil); }
		}
	}
