...
aifload(fname: string, adr: int)
{
	...
	fd := sys->open(fname, Sys->OREAD);
	if(fd == nil) 
		sys->raise(sprint("fail:%s:%r", fname));
	d: Sys->Dir;
	(nil, d) = sys->fstat(fd);
	b := array[d.length] of byte;
	sys->read(fd, b, d.length);
	if(adr < 0) {
		if(len b < 128)
			sys->raise(sprint("fail:%s:not aif", fname));
		tsize := intb(b[20:24]);
		dsize := intb(b[24:28]);
		bsize := intb(b[32:36]);
		tbase := intb(b[40:44]);
