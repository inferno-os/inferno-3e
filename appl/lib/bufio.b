...
create(filename: string, mode, perm: int): ref Iobuf
{
	...
	if ((fd := sys->create(filename, mode, perm)) == nil)
		return nil;
	ib := ref Iobuf(fd, array[Bufsize+Maxrune] of byte, 0, 0, 0, 0, 0, mode, mode);
	if (mode != OREAD)
...
fopen(fd: ref Sys->FD, mode: int): ref Iobuf
{
	...
	if ((filpos := sys->seek(fd, 0, 1)) < 0)
		filpos = 0;
	ib := ref Iobuf(fd, array[Bufsize+Maxrune] of byte, 0, 0, 0, filpos, filpos, mode, mode);
	if (mode != OREAD)
		flushbufs = ib :: flushbufs;
	return ib;
}
...
readchunk(b: ref Iobuf): int
{
	...
	if ((i := sys->read(b.fd, b.buffer[b.size:], len b.buffer - b.size)) <= 0) {
		if(i < 0)
			return ERROR;
		return EOF;
	}
