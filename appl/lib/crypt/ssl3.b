...
Handshake: adt {
	...
	ServerHelloDone =>
		...
		sha_hash 			: array of byte; # [20] Keyring->SHAdlen
	}
...
init(): string
{
	...
	if(n < 0) {
		...
		if(n < 0) {
			if(SSL_DEBUG)
				log(sys->sprint("ssl3: bind ssl device failed: %r\n"));
			exit;
		}
	}
	...
	ofs, n: int) : array of byte
{
	...
	md5 =>
		...
		ds0 := keyring->cloneDigestState(ms.ds[0]);
		if(ctx.status & SSL3_RECORD) {
			keyring->md5(b, len b, nil, ds0);
			keyring->md5(a[ofs:], n, digest, ds0);
			ds1 := keyring->cloneDigestState(ms.ds[1]);
	...
	sha =>
		...
		ds0 := keyring->cloneDigestState(ms.ds[0]);
		if(ctx.status & SSL3_RECORD) {
			keyring->sha(b, len b, nil, ds0);
			keyring->sha(a[ofs:], n, digest, ds0);
			ds1 := keyring->cloneDigestState(ms.ds[1]);
			keyring->sha(digest, len digest, digest, ds1);
		}
		else {
			keyring->sha(a[ofs:], n, nil, ds0);
			keyring->sha(b, len b, digest, ds0);
		}
	}			
	return digest;
}
...
set_queue(ctx: ref Context, q: ref RecordQueue, mac, key: array of byte): string
{
	...
	SSL_SHA =>
		...
		if(ctx.status & SSL3_RECORD) {
			ds = array [2] of ref DigestState;
			ds[0] = keyring->sha(mac, len mac, nil, nil);
			ds[1] = keyring->sha(mac, len mac, nil, nil);
			ds[0] = keyring->sha(SSL_MAC_PAD1, 40, nil, ds[0]);
			ds[1] = keyring->sha(SSL_MAC_PAD2, 40, nil, ds[1]);
		}
		else {
			ds = array [1] of ref DigestState;
			ds[0] = keyring->sha(mac, len mac, nil, nil);
		}
		q.macState = ref MacState.sha(Keyring->SHAdlen, ds);
	* =>
	...
	SSL_RC4 =>
		...
		rcs := setupRC4state(key, len key);		# RC4 should be in keyring really
...
update_handshake_hash(ctx: ref Context, r: ref Record): string
{
	...
	ctx.sha_state = keyring->sha(r.data, len r.data, nil, ctx.sha_state);
	...
	: (array of byte, array of byte)
{
	...
	sha_inner := array [Keyring->SHAdlen] of byte;
...
calc_key_material(n: int, ms, cr, sr: array of byte): array of byte
{
	...
	sha_value := array [Keyring->SHAdlen] of byte; # [20]
...
Handshake.decode(buf: array of byte): (ref Handshake, string)
{
	...
	SSL_HANDSHAKE_HELLO_REQUEST =>
		...
		if(len a != Keyring->MD5dlen + Keyring->SHAdlen) { # 16+20
			e = "finished: unexpected message";
			break;
		}
		md5_hash := a[i:i+Keyring->MD5dlen];	
		i += Keyring->MD5dlen;
		sha_hash := a[i:i+Keyring->SHAdlen];	
		i += Keyring->SHAdlen;
		if(i != len a) {
			e = "finished: unexpected message";
			break;
	...
	: (ref CipherSpec, ref KeyExAlg, ref SigAlg, string)
{
	...
	RSA_WITH_NULL_SHA => 	
		cip = ref CipherSpec(SSL_EXPORT_TRUE, SSL_NULL_CIPHER, 
			SSL_STREAM_CIPHER, 0, 0, SSL_SHA, Keyring->SHAdlen);
		kex = ref KeyExAlg.RSA(nil, nil, nil);
		sig = ref SigAlg.anon();
	...
	RSA_WITH_RC4_128_SHA => 	
		cip = ref CipherSpec(SSL_EXPORT_FALSE, SSL_RC4, 
			SSL_STREAM_CIPHER, 16, 0, SSL_SHA, Keyring->SHAdlen);
		kex = ref KeyExAlg.RSA(nil, nil, nil);
		sig = ref SigAlg.anon();
	...
	RSA_WITH_IDEA_CBC_SHA => 	
		cip = ref CipherSpec(SSL_EXPORT_FALSE, SSL_IDEA_CBC,
			SSL_BLOCK_CIPHER, 16, 8, SSL_SHA, Keyring->SHAdlen);
		kex = ref KeyExAlg.RSA(nil, nil, nil);
		sig = ref SigAlg.anon();

	RSA_EXPORT_WITH_DES40_CBC_SHA => 
		cip = ref CipherSpec(SSL_EXPORT_TRUE, SSL_DES_CBC, 
			SSL_BLOCK_CIPHER, 5, 8, SSL_SHA, Keyring->SHAdlen);
		kex = ref KeyExAlg.RSA(nil, nil, nil);
		sig = ref SigAlg.RSA(nil, nil);

	RSA_WITH_DES_CBC_SHA => 
		cip = ref CipherSpec(SSL_EXPORT_FALSE, SSL_DES_CBC, 
			SSL_BLOCK_CIPHER, 8, 8, SSL_SHA, Keyring->SHAdlen);
		kex = ref KeyExAlg.RSA(nil, nil, nil);
		sig = ref SigAlg.anon();

	RSA_WITH_3DES_EDE_CBC_SHA => 
		cip = ref CipherSpec(SSL_EXPORT_FALSE, SSL_3DES_EDE_CBC, 
			SSL_BLOCK_CIPHER, 24, 8, SSL_SHA, Keyring->SHAdlen);
		kex = ref KeyExAlg.RSA(nil, nil, nil);
		sig = ref SigAlg.anon();
	...
	DH_RSA_WITH_DES_CBC_SHA => 
		cip = ref CipherSpec(SSL_EXPORT_FALSE, SSL_DES_CBC, 
			SSL_STREAM_CIPHER, 8, 8, SSL_SHA, Keyring->SHAdlen);
		kex = ref KeyExAlg.DH(nil, nil, nil, nil, nil);
		sig = ref SigAlg.RSA(nil, nil);
	...
	FORTEZZA_KEA_WITH_FORTEZZA_CBC_SHA =>
		cip = ref CipherSpec(SSL_EXPORT_FALSE, SSL_FORTEZZA_CBC, 
			SSL_BLOCK_CIPHER, 0, 0, SSL_SHA, Keyring->SHAdlen);
		kex = ref KeyExAlg.FORTEZZA_KEA();
		sig = ref SigAlg.FORTEZZA_KEA();
...
md5_hash(input: list of array of byte, md5_ds: ref DigestState): (array of byte, ref DigestState)
{
	...
	if(md5_ds != nil)
		ds = keyring->cloneDigestState(md5_ds);
...
sha_hash(input: list of array of byte, sha_ds: ref DigestState): (array of byte, ref DigestState)
{
	...
	if(sha_ds != nil)
		ds = keyring->cloneDigestState(sha_ds);

	lab := input;
	for(i := 0; i < len input - 1; i++) {
		ds = keyring->sha(hd lab, len hd lab, nil, ds);
		lab = tl lab;
	}
	ds = keyring->sha(hd lab, len hd lab, hash_value, ds);
