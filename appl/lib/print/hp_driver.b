...
MASK := array[] of {byte 1, byte 3, byte 15, byte 255};
SHIFT := array[] of {7, 6, 4, 0};
GSFACTOR := array[] of {255.0, 255.0/3.0, 255.0/7.0, 1.0};
lastp : ref Printer;
...
sendimage(p: ref Printer, pfd: ref Sys->FD, display: ref Draw->Display, im: ref Draw->Image, width: int, lmargin: int, cancel: chan of int): int
{
	...
	outopen(pfd);
	dbg(sys->sprint("image ldepth=%d from %d,%d to %d,%d\n", im.ldepth, im.r.min.x, im.r.min.y, im.r.max.x, im.r.max.y));
	if (p != lastp) {
	...
	PCL_Start_Raster(1);
	cmap1 := setup_color_map(display, map1, im.ldepth);
	if (map2 != nil)
		cmap2 := setup_color_map(display, map2, im.ldepth);
...
setup_color_map(display: ref Display, map: array of KCMY, ldepth: int): array of KCMY
{
	gsfactor := GSFACTOR[ldepth];
	bpp := 1 << ldepth;
	max := 1 << bpp;
