...
{
	lineno = 1;
	return stylesheet(ref Cparse(-1, 0, nil, nil, Clex.new(s)));
}

parsedecl(s: string): (list of ref Decl, string)
{
	lineno = 0;
	return (declarations(ref Cparse(-1, 0, nil, nil, Clex.new(s))), nil);
}
...
{
	if(p.lookahead >= 0)
		sys->raise("css: internal error: Cparse.unget");
...
#	IDENT
# page:
#	"@page" IDENT? pseudo_page? '{' declaration [';' declaration]* '}'
# pseudo_page:
#	':' IDENT
# font_face:
#	"@font-face" '{' declaration [';' declaration]* '}'
#
# TO DO: @media, @page, @font_face
#
...
stylesheet(p: ref Cparse): (ref Stylesheet, string)
{
	...
	if(atkeywd(p, "@charset")){
		if(itisa(p, STRING)){
			...
			synerr("bad @charset declaration");
	}
			...
			if(c == IDENT){
				for(;;){
					...
					if(c != ',')
						break;
					if(!itisa(p, IDENT)){
						synerr("missing medium identifier");
						break;
					}
				}
			}
			imports = ref Import(name, rev(media)) :: imports;
		}else
			synerr("bad @import");
		if(c != ';'){
			synerr("missing ; in @import");
			p.unget(c);
			if(p.skipto(";}") < 0)
				break;
		}
	...
	revi := imports;
	for(imports = nil; revi != nil; revi = tl revi)
		imports = hd revi :: imports;

	rules: list of ref Statement;
	do{
		while((c := p.look()) == ATKEYWORD)	# skip unknown or misplaced at-rule
			skipatrule(p);
		(rule, err) := ruleset(p);
		if(rule == nil){
			if(err != nil){
				synerr(sys->sprint("bad ruleset: %s", err));
	...
	for(; rl != nil; rl = tl rl)
		rules = hd rl :: rules;
	return (ref Stylesheet(nil, imports, rules), nil);
}
...
{
	selectors: list of list of list of ref Select;
	c := -1;
	do{
		s := selector(p);
		if(s == nil){
	...
	if((c = p.get()) != '}'){
		synerr("unclosed declaration block");
	}
...
#	'+' S* | '>' S* | /* empty */
#
# TO DO: + and > combinators

selector(p: ref Cparse): list of list of ref Select
{
	sel: list of list of ref Select;
	...
	for(;;){
		...
		PSEUDO =>
			...
			FUNCTION =>
				...
				* =>
					synerr("bad pseudo-function syntax");
					p.unget(c);
					break Sel;
				}
				if((c = p.get()) != ')'){
					synerr("missing ')' for pseudo-function");
					p.unget(c);
					break Sel;
				}
			* =>
				synerr(sys->sprint("unexpected :pseudo: %s:%s", ptok(c), p.value));
				p.unget(c);
				break Sel;
			}
		* =>
...
term(p: ref Cparse, sep: int): ref Value
{
	...
	FUNCTION =>
		...
		c = p.get();
		if(c != ')'){
			synerr(sys->sprint("missing ')' for function %s", name));
			return nil;
		}
		if(name == "rgb"){
			if(len args != 3){
				synerr("wrong number of arguments to rgb()");
				return nil;
			}
			r := colourof(hd args);
...
Clex: adt {
	...
	new:	fn(s: string): ref Clex;
	getc:	fn(cs: self ref Clex): int;
	ungetc:	fn(cs: self ref Clex, c: int);
};

Clex.new(s: string): ref Clex
{
	return ref Clex(nil, s, len s, 0);
}
	...
	if(c != '"' && c != '\''){	# not a quoted string
		while(c != ' ' && c != '\n' && c != '\'' && c != '"' && c != ')'){
			...
			if(c == '\\'){
				c = cs.getc();
				if(c < 0){
					synerr("end of file in url parameter");
					break;
				}
				if(c == ' ' || c == '\'' || c == '"' || c == ')')
					s[len s] = c;
				else{
					synerr("invalid escape sequence in url");
					s[len s] = '\\';
					s[len s] = c;
				}
				c = cs.getc();
		...
		cs.ungetc(c);
#		if(s == nil)
#			synerr("empty parameter to url");
	}else
		s = quotedstring(cs, c);
	if((c = skipws(cs)) != ')'){
		synerr("unclosed parameter to url");
		cs.ungetc(c);
	}
	return s;
}
