...
memstart(): int
{
	...
	if(modl != nil && write(mkpath(PROF, CTL), "module " + modl) < 0)
		return -1;
	if(write(mkpath(PROF, CTL), "startmp") < 0)
		return -1;
	return 0;
}
...
stats(): Prof
{
	...
	d := array[1] of Sys->Dir;
	total := 0;
	while((nr := sys->dirread(fd, d)) > 0){
		if(d[0].name == CTL)
			continue;
		dn := mkpath(PROF, d[0].name);
...
coverage(p: Prof, v: int): Coverage
{
	...
	cleare();
	freq := v&FREQUENCY;	# always 0 for now
	for(ml := p.mods; ml != nil; ml = tl ml){
		mp := hd ml;
		if((b := getb(mp.path)) == nil)
			continue;
		...
		i = 1;
		ll := len line;
		llist: list of (list of (int, int, byte), string);
		while((s := bio.gets('\n')) != nil){
			f := 0;
			if(i < ll)
				f = line[i];
			rlist: list of (int, int, byte);
			if(rng != nil && i < ll && (r := rng[i]) != nil){
				for( ; r != nil; r = r.n){
					if(r.u == ∞)
						r.u = len s - 1;
					if(r.f&NEX)
						rlist = (r.l, r.u, byte ((r.f&EXE)==EXE)) :: rlist;
				}
			}
			else{
				if(f&NEX)
					rlist = (0, len s - 1, byte ((f&EXE)==EXE)) :: nil;
			}
			llist = (rlist, s) :: llist;
			i++;
		}
		clist = (b, mp.coverage, rev(llist)) :: clist;
	}
	return clist;
}
...
rev(llist: list of (list of (int, int, byte), string)): list of (list of (int, int, byte), string)
{
	r: list of (list of (int, int, byte), string);
