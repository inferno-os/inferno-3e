...
sprint : import sys;
Point, Rect, Font, Image : import drawm;Pointer : import Dat;
...
frinittick(f : ref Frame)
{
	...
	ft = f.font;
	f.tick = nil;
	f.tick = graph->balloc(((0, 0), (FRTICKW, ft.height)), (gui->mainwin).ldepth, Draw->White);
	if(f.tick == nil)
		return;
	f.tickback = graph->balloc(f.tick.r, (gui->mainwin).ldepth, Draw->White);
	if(f.tickback == nil){
		f.tick = nil;
		return;
	}
