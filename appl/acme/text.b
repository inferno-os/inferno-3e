...
Text.loadx(t : self ref Text, q0 : int, file : string, setqid : int) : int
{
	...
	if(d.qid.path & Sys->CHDIR){
		...
		dlp = nil;
		ndl = 0;
		dbuf = array[16] of Dir;
		while((n=sys->dirread(fd, dbuf)) > 0){
			for(i=0; i<n; i++){
				dl = ref Dirlist;
				dl.r = dbuf[i].name;
				if(dbuf[i].mode & Sys->CHDIR)
					dl.r = dl.r + "/";
...
Text.typex(t : self ref Text, r : int, echomode : int)
{
	...
	if(t.q1 > t.q0){
		...
		t.eq0 = ~0;
		if (r == 16r08){	# erase character : odd if a char then erased
			t.show(t.q0, t.q0);
			return;
		}
	}
...
xselect(f : ref Frame, mc : ref Dat->Pointer, col, colt : ref Image) : (int, int)
{
	p0, p1, q, tmp : int;
	mp, pt0, pt1, qt : Point;
	reg, b : int;
