...
eloginsert(f: ref File, q0: int, r: string, nr: int)
{
	...
	# try to merge with previous
	if(f.elog.typex==Insert && q0==f.elog.q0 && (q0+nr)-f.elog.q0<Maxstring){
		ofer := f.elog.r;
	...
	while(log.nc > 0){
		...
		* =>
		...
		Replace =>
			...
			#	f.curtext.replace(b.q0, b.q0+b.nd, buf.s, b.nr, TRUE, 0);
			#	break;
			# }
			f.curtext.delete(b.q0, b.q0+b.nd, TRUE);
			up -= b.nr;
			for(i=0; i<b.nr; i+=n){
				n = b.nr - i;
				if(n > BUFSIZE)
				...
				f.curtext.insert(b.q0+i, buf.s, n, TRUE, 0);
			}
			f.curtext.q0 = b.q0;
