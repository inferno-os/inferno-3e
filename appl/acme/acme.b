...
Point, Rect, Font, Image, Display : import drawm;
TRUE, FALSE, maxtab : import dat;
Ref, Reffont, Command, Timer, Lock, Pointer, Cursor : import dat;
...
keyboardtask()
{
	...
	if (sys->rescue("*", e) == Sys->EXCEPTION){
		if(e.name == nil){	# normal exit
			sys->rescued(Sys->ONCE, "");	
			acmeexit(nil);
		}
		else{
			shutdown(sys->sprint("%s (%s pc=0x%x)", e.name, e.mod, e.pc));
			sys->rescued(Sys->RAISE, e.name);
		}
	}
	...
	for(;;){
		...
		*mouse = <-cmouse =>
			...
			if(mouse.buttons == 1)
				...
				but = 3;
			else if (mouse.buttons == (M_DOUBLE|1))
				but = 1;
			barttext = t;
			if(t.what==Body && mouse.xy.in(t.scrollr)){
				if(but){
					if (mouse.buttons == (M_DOUBLE|1))
						mouse.buttons = 1;
					w.lock('M');
					t.eq0 = ~0;
					scrl->scroll(t, but);
			...
			if(mouse.buttons){
				...
				}else if(mouse.buttons & M_DOUBLE) {
					mouse.buttons &= ~M_DOUBLE;
					mouse.buttons |= 1;
					t.select(1);
					if(w != nil)
						w.settag();
...
iconinit()
{
	...
	button = balloc(r, mainwin.ldepth, Draw->White);
	...
	r = button.r;
	modbutton = balloc(r, mainwin.ldepth, Draw->White);
	...
	r = button.r;
	colbutton = balloc(r, mainwin.ldepth, Draw->White);
	...
	arrowcursor.hotspot = (-1, -1);
	arrowcursor.bits = balloc(r, 0, Draw->Black);
	arrowcursor.bits.writepixels(r, arrowbits);

	boxcursor = ref Cursor;
	boxcursor.hotspot = (-7, -7);
	boxcursor.bits = balloc(r, 0, Draw->Black);
	boxcursor.bits.writepixels(r, boxbits);
...
imagemix(i1 : ref Image, i2 : ref Image, n1 : int, n2 : int) : ref Image
{
	...
	i3 := balloc(((0, 0), (2, 2)), mainwin.ldepth, Draw->White);
