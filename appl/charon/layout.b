...
getfont(num: int) : ref Font
{
	...
	if(f == nil) {
		...
		if(f == nil) {
			if(num == DefFnt)
				CU->raise(sys->sprint("exLayout: can't open default font %s: %r", fonts[num].name));
			else {
...
drawline(f : ref Frame, layorigin : Point, l: ref Line, lay: ref Lay)
{
	...
	x := o.x;
	y := o.y;
	lr := Rect(Point(x, y), Point(x+l.width, y+l.height));
	inview := lr.Xrect(f.cr);
	...
	for(it := l.items; it != nil; it = it.next) {
		...
		Itext =>
			...
			if(i.ul != ULnone) {
				...
				if (ulw < 1)
					continue;
				im.draw(Rect(Point(ulx,yy),Point(ulx+ulw,yy+1)), fgi, nil, zp);
			}
		Irule =>
			if (!inview)
				break;
...
if (f.layout == nil)
sys->print("layout is nil, about to die, aaargh\n");
	newy = max(0, min(newy, f.totalr.max.y + f.layout.margin - f.cr.dy()));
...
Frame.scrollabs(f : self ref Frame, p : Point)
{
	...
	y = max(0, min(y, f.totalr.max.y + f.layout.margin - f.cr.dy()));
	if (x != f.viewr.min.x || y != f.viewr.min.y) {
		f.viewr.min = (x, y);
		fixframegeom(f);
		drawall(f);
	}
...
Frame.focus(f : self ref Frame, focus, raise : int)
{
	di := f.doc;
...
blurfocusevent(c: ref Control, kind, raise: int)
{
	if((CU->config).doscripts && c.ff != nil && c.ff.evmask) {
		if(kind == E->SEonblur)
			changeevent(c);
		if (!raise || !(c.ff.evmask & kind))
			return;
...
Control.losefocus(c: self ref Control, raise: int)
{
	if(c.flags & CFhasfocus) {
		c.flags &= ~CFhasfocus;
		if(c.f.cim != nil) {
			blurfocusevent(c, E->SEonblur, raise);
			c.draw(1);
		}
	}
}

Control.gainfocus(c: self ref Control, raise: int)
{
	if(!(c.flags & CFhasfocus)) {
		c.flags |= CFhasfocus;
		if(c.f.cim != nil) {
			blurfocusevent(c, E->SEonfocus, raise);
			c.draw(1);
		}
		G->clientfocus();
	}
