...
makeframeex(f : ref Layout->Frame) : (ref Exec, ref Obj)
{
	...
	reinitprop(scrobj, "colorDepth", ES->numval(real scr.ldepth));
	reinitprop(scrobj, "pixelDepth", ES->numval(real scr.ldepth));
...
evalscript(f: ref Layout->Frame, s: string) : (string, string, string)
{
	...
	if(sys->rescue("*", exc) == sys->EXCEPTION) {
		if(dbg) {
			sys->print("fatal error executing evalscript: %s\nscript=", ex.error);
			sa := array of byte s;
...
do_on(e: ref ScriptEvent)
{
	...
	if(sys->rescue("*", exc) == sys->EXCEPTION) {
		...
		if (exc.name == "throw") {
			...
			# fatal error
			top.error = 1;
			emsg := "Fatal error in script ("+exc.name+"):\n" + ex.error + "\n";
			G->alert(emsg);
		}
		if(e.reply != nil)
			e.reply <-= nil;
...
specindex(class: string) : int
{
	for(i := 0; i < len objspecs; i++)
		if(objspecs[i].name == class)
			break;
	if(i == len objspecs)
		CU->raise("EXInternal: couldn't find host object class " + class);
	return i;
}
