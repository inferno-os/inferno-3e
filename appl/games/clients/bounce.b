...
Ballstate: adt {
	owner: int;		# index into player array
	hitobs: ref Obstacle;
	t0: int;
	p, v: Realpoint;
	speed: real;
...
client(ctxt: ref Draw->Context, argv: list of string, nil: int)
{
	...
	gamecmd("time " + string sys->millisec());
...
#	sys->print("bounce: got update %s\n", s);
	...
	"set" =>
		...
		* =>
			...
			} else if (len attr > 6 && attr[0:6] == "player") {
				# playerN id colour
				n := int attr[6:];
				if (n >= len players) {
					newplayers := array[n + 1] of ref Player;
					newplayers[0:] = players;
					players = newplayers;
				}
				p := players[n] = ref Player(int hd val, hd tl val);
...
aim(mch: chan of (int, Point), hitbat: ref Obstacle, p: Point): (int, Point)
{
	...
	cmd(win, "update");
	(δ.x, δ.y) = (-δ.x, -δ.y);
	gamecmd("newball " + string hitbat.id + " " +
...
{
	displ := win.image.display;
	return cmd(win, ".c create oval 0 0 1 1 -fill " + players[owner].colour +
			" -tags o" + string owner);

#	col := <-randch & 16rffffff;
#	tag := "b" + string id;
#	cmd(win, "image create bitmap " + tag);
#	img := displ.newimage(ballmask.r, 3, 0, Draw->White);
#	img.draw(img.r, displ.rgb(col & 16rff, (col >> 8) & 16rff, (col >> 16) & 16rff), displ.ones, (0, 0));
#	tk->imageput(win, tag, img, ballmask);
#	cmd(win, ".c create image -100 -100 -image " + tag + " -tags " + tag);
#	return tag;
}
