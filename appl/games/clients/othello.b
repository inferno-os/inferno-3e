...
"label .status -text {No game in progress}",
"frame .f",
...
client1(ctxt: ref Draw->Context)
{
	...
	c := <-bcmd =>
		...
		"b1up" =>
			...
			if (!inboard)
				break;
			othellocmd(gamefd, "move " + string x + " " + string y);
			cmd(win, "update");
		}
	c := <-winctl =>
		if (c == "exit")
...
applyupdate(s: string)
{
	...
	"set" =>
		...
		case attr {
		"players" =>
			playerids[Black] = int hd tl toks;
			playerids[White] = int hd tl tl toks;
			status(string playerids[Black]+ "(Black) vs. " + string playerids[White] + "(White)");
			if (playerid == playerids[Black])
				cmd(win, ".f.l configure -text Black");
			else if (playerid == playerids[White])
				cmd(win, ".f.l configure -text White");
		"turn" =>
			turn := int val;
			if (turn != None) {
				if (playerid == playerids[turn])
					cmd(win, ".f.turn configure -text {(Your turn)}");
				else if (playerid == playerids[!turn])
