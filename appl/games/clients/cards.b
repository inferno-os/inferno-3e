...
"frame .buts",
"frame .cf",
"canvas .c -width 400 -height 450 -bg green -buffered 0",
"label .status -text 0",
...
"pack .buts.sizetofit .status -in .buts -side left",
...
client1()
{
	...
	sys->print("done config, text font is %s\n", Textfont);

	winctl: chan of string;
	(win, winctl) = wmlib->titlebar(drawctxt.screen, "-font " + Textfont,
		"Cards", Wmlib->Appl);
	cmd(win, ". unmap");
	bcmd := chan of string;
...
{
	r := b + "r";
	cmd(win, "grab ifunset .c");
	for (;;) {
		ev := <-bcmd;
		(n, toks) := sys->tokenize(ev, " ");
		if (hd toks == b) {
...
sizetofit()
{
	...
	cmd(win, ".c configure -scrollregion {0 0 " + p2s(csz) + "}");
...
pan(canvp, scrp: Point)
{
	...
	sz := s2r(cmd(win, ".c cget -scrollregion")).max;

	if (sz.x > 0)
		cmd(win, sys->sprint(".c xview moveto %f",  real co.x / real sz.x));
	if (sz.y > 0)
		cmd(win, sys->sprint(".c yview moveto %f", real co.y / real sz.y));
}
...
readimages(dir: string, prefix: string): (int, Point)
{
	...
	if (cardsize.x > 0 && cardsize.y > 0 &&
		...
		if (img.r.dx() % cardsize.x != 0 || img.r.dy() != cardsize.y)
			...
			for (i := 0; i < n; i++) {
				c := displ.newimage(((0, 0), cardsize), img.ldepth, 0, 0);
				c.draw(c.r, img, displ.ones, (x, 0));
				id := prefix + string i;
				cmd(win, "image create bitmap " + id);
				tk->imageput(win, id, c, nil);
				x += cardsize.x;
			}
			return (n, cardsize);
		}
...
animproc(a: ref Animation)
{
	...
	if (!srcpt.eq(Hiddenpos) && !dst.pos.eq(Hiddenpos) && !d.eq((0, 0))) {
		...
		while (t < dt) {
			...
			dp := p.sub(currpt);
			cmd(win, ".c move " + a.tag + " " + string dp.x + " " + string dp.y + ";update");
			currpt = p;
			yield();
			t = sys->millisec() - t0;
		}
...
readcardimages(): int
{
	...
	(nrears, rearsize) := readimages("/icons/cards", "rear");
