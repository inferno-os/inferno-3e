...
Cmdline.new(top: ref Tk->Toplevel, w, font: string): (ref Cmdline, chan of string)
{
	if (font != nil)
		font = " -font " + font;
	window_cfg := array[] of {
		...
		"text " + w + ".t -yscrollcommand {" + w + ".scroll set}" +font+" -height 5h -width 75w",
	...
	if (err != nil) {
		...
		sys->raise("fail:commandline config error");
	}
...
Cmdline.addtext(cmdl: self ref Cmdline, txt: string)
{
	...
	add(cmdl, wmlib->tkquote(txt) + " {}" , len txt);
}
