...
srv(newclientch: chan of (int, ref Styxserver, string), tmsgch: chan of (int, ref Styxserver, ref Tmsg))
{
...
newplayer(game: ref Game, ofid: ref Openfid): string
{
	...
	ofid.player = player;
	if ((err := game.mod->join(player)) != nil) {
		ofid.player = nil;
		return err;
	}
...
newgame(client: ref Client): ref Game
{
	for (i := 0; i < len games; i++)
		if (games[i] == nil)
			break;
	if (i == len games) {
		newgames := array[len games + 1] of ref Game;
		newgames[0:] = games;
		games = newgames;
	}
...
Client.cmd(client: self ref Client, ofid: ref Openfid, cmd: string): string
{
	...
	fGAME =>
		...
		if (ofid.player == nil)
			...
			err = game.mod->command(ofid.player, cmd);
	fPLAYERS =>
		for (i := 0; i < len cmd; i++)
			if (cmd[i] == '\n')
				break;
...
inactivegamecmd(client: ref Client, g: ref Game, ofid: ref Openfid, cmd: string): string
{
	...
	g.name = title;
	if ((err := g.mod->init(g, myself)) != nil || (err = newplayer(g, ofid)) != nil) {
		g.mod = nil;
		g.objects = nil;
		return err;
	}
