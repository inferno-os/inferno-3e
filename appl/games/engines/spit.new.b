...
command(p: ref Player, cmd: string): string
{
	...
	SPIT =>
		if (awaitingslap)
			return "gotta slap a pile first";
		if (!dealt) {
			deal();
			dealt = 1;
		} else if (!playing) {
	...
	CLICK =>
		...
		if (sel.isempty() || sel.stack == stack) {
			...
			if (awaitingslap) {
				if (stype != "centre")
					return "can't slap there";
				finish(owner(stack).ord ^ cp.ord);
				awaitingslap = 0;
				return nil;
			}
...
# finish a round; slapped is the pile that player 0 gets.
finish(slapped: int)
{
	c0 := len dplayers[0].centre.children;
	c1 := len dplayers[1].centre.children;
	if (c0 == c1)
		remark("equal piles");
	else {
		winner := (c0 > c1) != slapped;
		game.action("remark bother!", nil, nil, 1<<(Cplayer.index(!winner).id));
		game.action("remark got it!", nil, nil, 1<<(Cplayer.index(winner).id));
	}

	winner: ref Player;
	for (i := 0; i < 2; i++) {
		bottomdiscard(dplayers[slapped].centre, dplayers[i].spare);
		d := dplayers[i];
		for (j := 0; j < len d.row; j++)
			bottomdiscard(d.row[j], d.spare);
		if (len d.spare.children == 0)
			winner = Cplayer.index(i).p;
		slapped = !slapped;
	}
	playing = 0;
	dealt = 0;
...
go()
{
	# if one player's out of spares, then only one card
	# gets dealt.
	(s1, s2) := (0, 2);
	if (len dplayers[0].spare.children == 0 || len dplayers[1].spare.children == 0) {
		loser := len dplayers[0].spare.children == 0;
		(s1, s2) = (loser, loser + 1);
	}
	for (i := s1; i < s2; i++) {
		d := dplayers[i];
