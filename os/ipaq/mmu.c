...
pr16(ulong n)
{
	...
	for(i=28; i>=0; i-=4)
		serputc("0123456789ABCDEF"[(n>>i)&0xF]);
}
...
mmuinit(void)
{
	...
	for(i=0; i<32*MB; i+=MB)
		ttb[MmuL1x(FLASHMEM+i)] = i | 0xC10 | MmuL1section;	/* we'll make flash uncached for now */

	/*
	 * build page table for ALT_IVEC page, mapping trap vectors in *page0
	 */
	ptable = xspanalloc(SectionPages*sizeof(*ptable), PtAlign, 0);
	ptable[MmuL2x(ALT_IVEC)] = PADDR(page0) | MmuL2AP(MmuAPsrw) | MmuWB | MmuIDC | MmuL2small;
	ttb[MmuL1x(ALT_IVEC)] = PADDR(ptable) | MmuL1page;
	mmuenable(KTTB);
	mmuregw(CpDAC, 1);
	mmuctlregw(mmuctlregr() | CpCDcache | CpCwb | CpCi32 | CpCd32 | CpCIcache | CpCaltivec);
}
...
{
	wbflush(a, n);
	flushicache();	/* can't be more precise */
	return 0;
}
