...
pcmciareset(void)
{
	...
	for(slotno = 0; slotno < Maxslot; slotno++){
		...
		slotdis(pp);
		//g->gedr = GPIOall[slotno];	/* ??? */
		intrenable(pcmpin(slotno, PCMeject), pcmciaintr, 0, BusGPIOrising);
		if((v = pcmpin(slotno, PCMstschng)) >= 0)	/* status change interrupt */
			intrenable(v, pcmciaintr, 0, BusGPIOrising);
	}
}
...
ns2count(int ns)
{
	...
	y = 100000000/(conf.cpuspeed/1000);
...
void
outb(ulong p, uchar v)
{
	*(uchar*)p = v;
}

void
outs(ulong p, ushort v)
{
	*(ushort*)p = v;
}
