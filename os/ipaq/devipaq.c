...
ipaqreset(void)
{
	intrenable(Powerbit, powerintr, nil, BusGPIOfalling);
}
...
static int
ipaqwalk(Chan* c, char* name)
{
	return devwalk(c, name, ipaqtab, nelem(ipaqtab), devgen);
}

static void
ipaqstat(Chan* c, char* db)
{
	devstat(c, db, ipaqtab, nelem(ipaqtab), devgen);
}
...
static long
ipaqread(Chan* c, void* a, long n, ulong offset)
{
	char *tmp, buf[64];
	uchar reply[12];
	int v, p, l;
...
static long
ipaqwrite(Chan* c, void* a, long n, ulong)
{
...
touched(Block *b, int buttons)
{
	...
	if(BLEN(b) == 5){
		...
		mousetrack(buttons&7, dx, dy);	/* TO DO: allow more than 3 buttons */
		touch.down = 1;
	}else{
		if(touch.down){
			mousetrack(0, 0, 0);	/* stylus up */
			touch.down = 0;
		}else
			touch.down = -1;
		touch.n = 0;
