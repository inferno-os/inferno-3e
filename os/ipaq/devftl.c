...
int
ftlgen(Chan *c, Dirtab */*tab*/, int /*ntab*/, int i, Dir *dp)
{
	int n;
	switch (i) {
	case 0:
		devdir(c, (Qid){Qctl, 0}, "ftlctl", 0, eve, 0666, dp);
		break;
	case 1:
		devdir(c, (Qid){Qdata, 0}, "ftldata", ftls ? ftls->rwlimit * Bsize : 0, eve, 0666, dp);
		break;
	default:
		if (ftls == nil)
			return -1;
		...
		devdir(c, (Qid){Qdata + 1 + n, 0}, ftls->part[n].name, ftls->part[n].size, eve, 0666, dp);
	}
	return 1;
}
...
{
	return devwalk(c, name, 0, 0, ftlgen);
}

static void	 
ftlstat(Chan *c, char *dp)
{
	devstat(c, dp, 0, 0, ftlgen);
}
	...
	if(DATAQID(c->qid.path)){
		...
		if(i >= 2 && (strcmp(fields[0], "init") == 0 || strcmp(fields[0], "format") == 0)){
			if(i > 2)
				...
				base = 0x100000;	/* TO DO: hunt for signature */
			if(i > 3)
...
{
	int i, j, nov, segblocks, n, badseg;
	ulong limit;
	Terase *e;
	Ftl *ftl;
	char buf[48], *fields[6];
	...
	if(ftl->flash == nil)
		error(up->env->error);
	ftl->flashctl = ftlkopen(fname, "ctl", OWRITE);
	if(ftl->flashctl == nil)
		error(up->env->error);
	ftl->flashstat = ftlkopen(fname, "stat", OREAD);
	if(ftl->flashstat == nil)
		error(up->env->error);
	ftl->flashstat->offset = 0;
	if((n = kchanio(ftl->flashstat, buf, sizeof(buf), OREAD)) <= 0){
		print("ftl: stat failed: %s\n", up->env->error);
		error(up->env->error);
	}
	else {
		ulong e;
		...
		if(n >= 4 && (e = strtoul(fields[3], nil, 16)) >= 16*1024){
			for (i=0; i < 32 && (1<<i) != e; i++)
				;
			if (eshift)
	...
	if (base == Nolimit) {
		...
		print("ftl: partition at 0x%.8lux, length 0x%.8lux\n", base, size);
