 ...
 * Revisions Copyright © 2000 Vita Nuova Holdings Limited.  All rights reserved.
 */
...
enum {//18000000
	...
	DORESET = 0,		/* send soft-reset during initialisation */
	DEBUG = 0,
};

#define	IOREG(r) (IsaIOBase+((IOBase+(r))<<2))
...
attach(Ether *ether)
{
	...
	/* BUG: intreable should set up the following */
	mask = 1 << ether->irq;
	s = splhi();
	GPIOREG->gfer &= ~mask;
	GPIOREG->grer |= mask;	/* level triggered, active high */
	GPIOREG->gpdr &= ~mask;
	GPIOREG->gafr &= ~mask;
	GPIOREG->gedr = mask;
	splx(s);
...
interrupt(Ureg*, void *arg)
{
	...
	ether = arg;
	ctlr = ether->ctlr;
	GPIOREG->gedr = 1<<ether->irq;	/* reset now, for level triggered */
	lock(ctlr);
...
reset(Ether* ether)
{
	...
	*(ushort*)IsaIOBase = 0xDEAD;	/* force rubbish on bus */
	...
	if(memcmp(ea, ether->ea, Eaddrlen) == 0)
		return -1;		/* ethernet address unknown: could take from EEPROM if there were one */
