	...
	case Qctl:
		...
		if(i >= 2 && (strcmp(fields[0], "init") == 0 || strcmp(fields[0], "format") == 0)){
			...
			if((ftl = ftlget()) != nil){
				...
				error(Enodev);
		}else if(strcmp(fields[0], "sync") == 0){
			if(ftls != nil)
				syncflash(ftls);
		}else
			error(Ebadarg);
...
mkftl(char *fname, ulong base, ulong size, int eshift, char *op)
{
	...
	if((n = kchanio(ftl->flashstat, buf, 32, OREAD)) <= 0){
	...
	if(kchanio(ftl->flashctl, "ftlerase", 8, OWRITE) <= 0){
		print("ftl: ftlerase failed: %s\n", up->env->error);
		error(up->env->error);
	}
...
syncflash(Ftl *ftl)
{
	if(kchanio(ftl->flashctl, "sync", 4, OWRITE) <= 0){
		print("ftl: flash sync error %s\n", up->env->error);
		error(up->env->error);
	}
}
