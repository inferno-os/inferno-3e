...
atainit(void)
{
	...
	DPRINT("ATA intr %lud\n", cp->sairq);
	intrenable(cp->sairq, ataintr, cp, BusGPIO);
	dp = ata;
	if(equip & 0xf0){
		dp->drive = 0;
		dp->online = 0;
...
ataxfer(Drive *dp, Partition *pp, int cmd, long start, long len, char *buf)
{
	...
	DPRINT1("<%s %ld>", (cmd == Cwrite) ? "W" : "R", lblk);
	cp = dp->cp;
	qlock(cp);
	if(waserror()){
		cp->buf = 0;
...
print("\nata%d model %s with %d lba sectors\n", dp->drive, id, dp->sectors);/**/
		...
		dp->heads = ip->heads;
		dp->sectors = ip->s2t;
print("\nata%d model %s with default %ld cyl %d head %d sec\n", dp->drive,
		...
		if(ip->cvalid&(1<<0)){
			...
			dp->heads = ip->cheads;
			dp->sectors = ip->cs2t;
print("\tchanged to %ld cyl %d head %d sec\n", dp->cyl, dp->heads, dp->sectors);/**/
		}
