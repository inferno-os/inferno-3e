...
touchintr(Ureg*, void*)
{
  	...
  	if (GPIOREG->gedr&BIT(gpio_irq_ucb1200)) {
		mcptouchintrdisable();
    		GPIOREG->gedr = BIT(gpio_irq_ucb1200); 	/* Clear the edge detect. */
		pen = DOWN;
   		wakeup( &touchRendez );
	}
}
...
touchproc()
{
	...
	GPIOREG->grer |= BIT(gpio_irq_ucb1200);
...
touchinit(void)
{
	...
	if (touchenabled)
		return;
	touchenabled = 1;
	kproc( "touchscreen", touchproc, 0 , 0);
}
...
static long	 
touchwrite(Chan* c, void* va, long n, ulong)
{
	char buf[256];
	char *cp, *a;
	int n0 = n;
