...
tcpsetstate(Conv *s, uchar newstate)
{
	...
	print( "%d/%d %s->%s CurrEstab=%d\n", s->lport, s->rport,
		tcpstates[oldstate], tcpstates[newstate], tpriv->stats[CurrEstab] );
	**/
...
/*
 *  tcpclose is always called with c locked
 */
static void
tcpclose(Conv *c)
{
...
Block *
htontcp(Tcp *tcph, Block *data, Tcphdr *ph, Fs *f)
{
	int dlen;
	Tcphdr *h;
	ushort csum;
...
int
ntohtcp(Tcp *tcph, Block **bpp, Fs *f)
{
	Tcphdr *h;
	uchar *optr;
	ushort hdrlen;
 ...
 */
void
sndrst(Proto *tcp, uchar *source, uchar *dest, ushort length, Tcp *seg, int)
{
	Tcphdr ph;
	Block *hbp;
	uchar rflags;
...
tcphangup(Conv *s)
{
	...
	if(s->raddr != 0) {
		...
		hnputs(ph.tcplen, TCP_HDRSIZE);
		hbp = htontcp(&seg, nil, &tcb->protohdr, s->p->f);
...
tcpiput(Proto *tcp, uchar*, Block *bp)
{
	...
	if(s == nil && (seg.flags & SYN)) {
		...
		if(seg.flags & ACK) {
			qunlock(tcp);
			sndrst(tcp, source, dest, length, &seg, RST_BOGUS);
			freeblist(bp);
			return;
		}
	...
	if(s == nil) {
		qunlock(tcp);
		sndrst(tcp, source, dest, length, &seg, RST_RXSYN1);
		freeblist(bp);
		return;
	}
	...
	switch(tcb->state) {
	case Closed:
		sndrst(tcp, source, dest, length, &seg, RST_CLOSED1);
		goto raise;
	case Listen:
		if(seg.flags & SYN) {
			procsyn(s, &seg);
	...
	case Syn_sent:
		if(seg.flags & ACK) {
			if(!seq_within(seg.ack, tcb->iss+1, tcb->snd.nxt)) {
				sndrst(tcp, source, dest, length, &seg, RST_TXSYN);
				goto raise;
			}
		}
		if(seg.flags & RST) {
	...
	if(length && tcb->state == Closed) {
		sndrst(tcp, source, dest, length, &seg, RST_CLOSED2);
		goto raise;
	}
	...
	for(;;) {
		...
		case Syn_received:
			if(!seq_within(seg.ack, tcb->snd.una+1, tcb->snd.nxt)){
				sndrst(tcp, source, dest, length, &seg, RST_RXSYN2);
				goto raise;
			}
			update(s, &seg);
			tcpsetstate(s, Established);
		...
		if(length == 0) {
			...
			case Finwait2:
				...
				if(bp != nil)
					freeblist(bp);
				sndrst(tcp, source, dest, length, &seg, RST_FINWAIT2);
				qunlock(s);
				poperror();
				return;
			}
...
/*
 *  always enters and exits with s locked.  We drop
...
/*			netlog(f, Logtcp, "qcopy: dlen %d blen %d sndcnt %d qlen %d sent %d rp[0] %d\n",
		...
		hbp = htontcp(&seg, bp, &tcb->protohdr, f);
		if(hbp == nil) {
			freeblist(bp);
			return;
		}
...
tcpsendka(Conv *s)
{
	...
	/* Build header, link data and compute cksum */
	hbp = htontcp(&seg, dbp, &tcb->protohdr, s->p->f);
	if(hbp == nil) {
		freeblist(dbp);
		return;
	}
