...
conswrite(Chan *c, void *va, long n, ulong offset)
{
	...
	case Qcaphash:
		if(capwritehash(va, n) < 0)
			error(Ebadarg);
		return n;

	case Qcapuse:
		if(capwriteuse(va, n) < 0)
			error(Eperm);
		return n;
...
sysctlwrite(char *cmd)
{
	...
	if (nf >= 1) {
		...
		if (nf == 2 && strcmp(fields[0], "console") == 0){
			if (strcmp("print", fields[1]) == 0) {
				if (consolepush < (1<<29))
					consolepush = (consolepush << 1) | consoleprint;
				consoleprint = 1;
			} else if (strcmp("noprint", fields[1]) == 0) {
				if (consolepush < (1<<29))
					consolepush = (consolepush << 1) | consoleprint;
				consoleprint = 0;
			} else if (strcmp("restore", fields[1]) == 0) {
				if (consolepush != 1){
