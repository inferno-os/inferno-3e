...
Sys_export(void *fp)
{
	...
	f = fp;
	release();
	*f->ret = export(fdchk(f->c), nil, f->flag&Sys_EXPASYNC);
	acquire();
}

void
Sys_exportdir(void *fp)
{
	F_Sys_exportdir *f;
...
Sys_pctl(void *fp)
{
	...
	p = currun();
	if (f->flags & (Sys_NEWFD|Sys_FORKFD|Sys_NEWNS|Sys_FORKNS|Sys_NEWENV|Sys_FORKENV))
		release();
	o = p->osenv;
	if(f->flags & Sys_NEWFD) {
		...
		for(l = f->movefd; l != H; l = l->tail) {
			fd = *(int*)l->data;
			if(fd >= 0 && fd <= ofg->maxfd && fd <= nfg->maxfd) {
				c = ofg->fd[fd];
				if(c != nil && nfg->fd[fd] == nil) {
					incref(c);
					nfg->fd[fd] = c;
					if(nfg->maxfd < fd)
						nfg->maxfd = fd;
