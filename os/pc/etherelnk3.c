...
statistics(Ether* ether)
{
	...
	uf = inb(port+UpperFramesOk) & 0xFF;
	ctlr->stats[FramesXmittedOk] += (uf & 0x30)<<4;
	ctlr->stats[FramesRcvdOk] += (uf & 0x03)<<8;
...
receive905(Ether* ether)
{
	...
	for(pd = ctlr->uphead; pd->control & upPktComplete; pd = pd->next){
		...
		else if(bp = iallocb(ROUNDUP(sizeof(Etherpkt)+4, 4))){
