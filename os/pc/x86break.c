...
static Brk *skip;
int breakmatch(BrkCond *cond, Ureg *ur, Proc *p);
void breaknotify(Brk *b, Proc *p);
void breakrestore(Brk *b);
Brk* breakclear(int id);
...
machbreakinit(void)
{
	...
	intrenable(1, skiphandler, 0, BUSUNKNOWN);
}
...
breakhit(Ureg *ur, Proc *p)
{
	...
	for(b = breakpoints; b != (Brk*) nil;  b = b->next) {
		if(b->addr == ur->pc) {
