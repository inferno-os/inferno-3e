...
typedef enum { None, Calibrate, Base, Op, Intr, Dis, Gc, MS2T};
static struct {
...
log(char *msg, ...)
{
	...
	va_start(ap, msg);
	bench.wpos = doprint(bench.wpos, bench.buf+bench.bufsz, msg, ap);
	va_end(ap);
}
...
cal(int report, void *va, long n, ulong offset)
{
	...
	US2TS = MS2TS/1000;
	if(va)
		log("loopconst=%lud mdelay=%lud lim=%lud tot=%lud low=%lud max=%lud\n", m->loopconst, mdelay, lim, tot, low, max);
	setpri(PriNormal);
	return n;
}
...
intrwake(Ureg*, void*)
{
	...
	if(spltbl == 0)		// not used here
		curct->spllo = curct->intr = curct->isave = (*ts32)();
	else {
		curct->spllo = spltbl;
		curct->intr = intrtbl;
...
intrtime(int report, void *va, long n, ulong offset)
{
	...
	curct = counters;
	ienable = 0;
	intrenable(VectorCLOCK, intrwake, nil, BUSUNKNOWN);
...
mlookup(Module *mod)
{
	...
	for(m=vmstat.hash[hash(mod->name, NAMELEN)]; m; m=m->hash)
	...
	if(m == 0)
		return 0;
	memmove(m->name, mod->name, NAMELEN);
	m->path = strdup(mod->path);
	m->ninst = mod->nprog;
...
enum {
	...
	Benchctlqid,
	Benchusqid,
	Nbenchtab=Benchusqid,
	Benchmaxqid,
};
#define Data 0
static Dirtab benchtab[Nbenchtab]={
	"bdata",	{Benchdataqid},		0,	0444,
	"bctl",	{Benchctlqid},		0,	0666,
	"busec",	{Benchusqid},		0,	0666,
};
...
static int
benchwalk(Chan *c, char *name)
{
	return devwalk(c, name, benchtab, (long)Nbenchtab, devgen);
}
...
static void
benchstat(Chan *c, char *db)
{
	switch(c->qid.path & ~CHDIR){
	case Benchdataqid:
		benchtab[Data].length = bench.wpos - bench.buf;
	}
	devstat(c, db, benchtab, (long)Nbenchtab, devgen);
}
...
static long	 
benchread(Chan *c, void *buf, long n, ulong offset)
{
	vlong us;
	char tmp[64];
	...
	switch(c->qid.path & ~CHDIR){
	case Benchdirqid:
		return devdirread(c, buf, n, benchtab, Nbenchtab, devgen);
...
static long	 
benchwrite(Chan *c, void *buf, long n, ulong offset)
{
	int argn = n;
