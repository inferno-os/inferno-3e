...
atagen(Chan* c, Dirtab*, int, int s, Dir* dirp)
{
	...
	JPRINT("atagen: %s%s %ulx - %ulx %d %ullx\n", 
		dp->vol, pp->name, pp->start, pp->end, dp->bytes, l);
...
//skipslave:
	...
	intrenable(VectorPIC+irq, ataintr, ctlr, ctlr->tbdf);
	inb(cmdport+Pstatus);
	outb(ctlport+Pctl, 0);
	if(devp->ienable)
		devp->ienable(devp);
...
static long
ataread(Chan* c, void* a, long n, ulong off)
{
	Drive *dp;
	long rv, i;
	int skip;
	...
	JPRINT("ataread : off=%ulx n=%ld a=%p\n", off, n, a);
	if (dp->atapi) {
		JPRINT("atapi !!!!!\n");
		return 0;
	}
	...
	skip = off % dp->bytes;
	for(rv = 0; rv < n; rv += i){
		JPRINT("atareadloop: rv=%d i=%d skip=%d n=%d off=%d buf=%p\n",
			rv, i, skip, n, off, buf);
		if(dp->atapi)
...
static long
atawrite(Chan *c, void *a, long n, ulong off)
{
	Drive *dp;
	long rv, i, partial;
	uchar *aa = a;
...
ataxfer(Drive *dp, Partition *pp, int cmd, vlong off, long len, uchar *buf)
{
	...
	JPRINT("ataxfer : cmd=%d off=%ullx len=%d buf=%p\n", cmd, off, len, buf);
