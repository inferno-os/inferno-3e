...
struct error_check {
	char global_too_large[(ulong)(G+1) < 0x4000];
};
...
main(ulong himem)
{
	...
	print("\n\4 \2.\2\3  \2MHz himem:\1 flash:\1 auto:\1\n",
...
bppoll(int ms)
{
	...
	for(;;) {
		...
		if(!g->bp.recv(buf, 3, 1)) {
			unblock(buf);
			if(!g->bp.recv(buf, 3, ms))
				break;
		}
		convM2S((char*)buf, &g->fc, 3);
		g->bp.recv(buf, 0, 0);
		if(!fcall(buf, &g->fc)) {
			g->bps = DEFBPS;
			g->bp.bps = 0;
...
call(void *addr, Bpi *bp)
{
	...
	// now that the line should be silent,
	// kill the DMA channel, in case the called program wants
	// to use it.  If the called program wants to continue using
	// StyxMon, then it should quickly call bpi->poll(1) to
	// re-enable the DMA before any messages get dropped
	DMAHW(DMA_CHAN)->dcsr = DCSR_RUN;
