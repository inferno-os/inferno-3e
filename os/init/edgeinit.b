implement Init;
#
# init program for NuovaPlay kernel
#
include "sys.m";
sys: Sys;
...
init()
{
	...
	if (0) {
		...
		if (srv != nil)
			if (remotefs(srv) < 0)
				...
				bind("/n/remote/usr/interplay", "/usr/interplay", sys->MREPL);
			}
	} else {
	...
	print("Local Disk...\n");
	xfs := load Shell "/dis/svc/dossrv/dossrv.dis";
	if(xfs == nil)
		print("No local disk filesystem: %r\n");
	else
		xfs->init(nil, "dossrv"::"-f"::"#H/hd2disk"::"-m"::"/n/disk"::nil);
	...
	print("DirectPlay Games support...\n");
	chdir("/usr/interplay/np");
	np := load Shell "nuovaplay.dis";
	if(np == nil)
		print("Games support not loaded\n");
	else
		spawn np->init(dc, "np" :: "200.1.1.61!15899" :: nil);

	print("Quake support...\n");
	qm := load Shell "/usr/interplay/qmaster/qmaster.dis";
	if(qm == nil)
		print("Quake support not loaded\n");
	else
		spawn qm->init(dc, "qmaster" :: nil);
